...
5. `user_profile` 是长期语义状态，拥有 evidence log、版本化 Profile、Dreaming queue、
   ...
   compaction 触发。
6. `group_profile` 是群聊可见的派生状态边界。它不读取私人 `UserProfile`，未来由群消息
   evidence 独立形成 `GroupProfile`、`GroupMemberProfile` 与 `GroupTopicProfile`；当前版本
   先实现群感知 Context 与读取边界，不提前建立没有 producer 的空表或后台 worker。
...
domain.group_profile -----> domain.temporal only
...
- `group_profile` 读取或反向引用私人 `user_profile`；
...
### 3.5 群感知 Context 与复合群画像

群聊不是“某个用户的私聊换了一个 chat ID”，而是一个多参与者共享事实流。Telegram
Conversation identity 因而按地址而不是发送者建立：

```text
private       assistant-user:{user_id}
group main    assistant-group:{group_id}:thread:0
group topic   assistant-group:{group_id}:thread:{message_thread_id}
```

同一群 Topic 的所有成员 Turn 进入同一 append-only Conversation、同一 Context Window 与同一
compaction epoch；每条 user message 必须保留 `user_id`、display name、username 与 Topic，避免
模型把多人话语错误归因给当前说话者。推理 activity 继续按 `conversation_id` 严格因果串行，
因此不同成员不会并发消费同一个旧 Context snapshot。

常驻 Context 只携带“Bot 实际参与过的 speaker-aware Conversation tail”。更广的旁观群消息仍在
canonical `group_message_projection` 中，通过非缓存 `fetch_group_context` 临时换入；该工具由
durable scope 强制群与 Topic，不允许模型传 `group_id`，结果只驻留当前 Agent Turn，且受独立
“默认 256 条、最多 512 条 + 16,384 token”双硬边界约束。条数边界只防极端碎片化，token
边界负责实际体积，因此 100 条人类短句不会被过早截断。两者分别对应 resident working set 与
query-time page-in，不重复写入 Conversation，也不重复参与 compaction。

共享 Conversation 的 resident Context Window 同样执行双水位：默认超过 512 条或 114,000 token
即异步触发 compaction，超过 768 条或 120,000 token 则等待 checkpoint；这防止大量极短消息绕过
纯 token 限制，同时为真实短对话保留足够大的 recent tail。

私人 `UserProfile`、`personal_info` 与 diary 在群 acceptance 路径上禁止读取，`ContextState`
构造器再次执行 fail-closed 校验。群画像采用三个正交投影而不是折衷成一个混杂文档：

| 投影 | 身份 | 只回答 | 证据边界 |
|---|---|---|---|
| `GroupProfile` | `group_id` | 群的长期主题、项目、规范、共享术语 | 该群消息 |
| `GroupMemberProfile` | `(group_id, user_id)` | 此人在该群公开表现出的角色、专长、承诺、互动风格 | 该成员在该群的消息及公开互动 |
| `GroupTopicProfile` | `(group_id, thread_id)` | 当前 Topic 的目标、决定、约束、未决问题 | 该群该 Topic 消息 |

一次群 Turn 在 acceptance 冻结的复合读取模型为：

```text
GroupContextProfile =
    GroupProfile(group_id)
  + GroupTopicProfile(group_id, thread_id)
  + GroupMemberProfile(group_id, current_user_id)
  + bounded relevant GroupMemberProfile[]
```

`relevant` 只允许当前消息明确提及者与最近活跃 speaker，使用稳定 tie-break 且设很小上限；禁止把
全群成员画像无界塞入 Context。三个投影未来共享同一群 evidence log，但各自拥有独立
`projection_target + revision + watermark + lease + fencing`。同一消息可合法支持多个语义投影，
因为这是不同 consumer 的不同派生状态；同一投影 target 内则只能推进一次 watermark。

隐私信息流是单向偏序（privacy lattice），不是运行时“看情况混合”：

```text
PRIVATE UserProfile(user)
    -- explicit, revocable ProfileDisclosure(user, group, claim_key) -->
GROUP_MEMBER(group, user)

GROUP evidence --> GROUP / GROUP_MEMBER / GROUP_TOPIC
GROUP_* -X-> PRIVATE UserProfile
PRIVATE UserProfile -X-> group Context by default
```

`ProfileDisclosure` 只是未来扩展位；在它具备用户可见授权、claim 级白名单、撤回与审计之前，默认
路径保持完全关闭。这样，Assistant 可以在不同群对同一人形成不同的公开印象，也不会泄露其私聊
偏好、日记或个人信息。

Telegram Bot API 的可观测性是该语义的前置条件：默认 Privacy Mode 只投递与 Bot 相关的群消息。
若产品承诺“所有群成员消息”，部署必须让 Bot 成为管理员或关闭 Privacy Mode，并在群成员列表与
产品说明中明确告知；否则系统只能诚实地称为“所有已观测群消息”。
...
12. `0046` 将 Telegram 群与 Topic 提升为 Conversation identity，保存 speaker identity，
    建立 Topic-local canonical group projection，并禁止群 acceptance 读取私人状态。
...
必要时二分截断并显式标记。自动召回默认 64 条、领域硬上限 128 条；显式 `search_memory`
同样默认 64 条、最多 128 条并另受 4096-token JSON 回填限制。条数上限负责碎片化与数据库工作量，
token 上限负责模型输入体积。checkpoint replay 没有模型调用，因而不检索。隔离的 translation
...
- 同一群 Topic 的不同成员必须共享 Conversation/Context Window，且每条历史保留 speaker identity；
- 群 Topic 彼此拥有独立 Conversation 与 compaction epoch，ambient context tool 不得跨 Topic；
- 群 acceptance 不得读取或冻结私人 User Profile、personal info 或 diary；
- `GroupContextProfile` 未来只能由 group-scoped evidence 形成，私人 claim 只能经显式 disclosure 投影；
...
- [Telegram Bot Features: Privacy Mode](https://core.telegram.org/bots/features#privacy-mode)：
  默认群 Bot 只接收相关消息；管理员或关闭 Privacy Mode 才能观察普通群消息。
- [Matrix Client-Server API: room events](https://spec.matrix.org/latest/client-server-api/#room-events)：
  成熟多方消息系统以 room-scoped timeline、sender 与 event relationship 表达共享事实流。
- [Slack `conversations.replies`](https://docs.slack.dev/reference/methods/conversations.replies/)：
  生产系统以 conversation + thread timestamp 建立局部有序讨论边界。
- [Qamar et al., *Who is Speaking?*, Findings of EMNLP 2023](https://aclanthology.org/2023.findings-emnlp.678/)：
  多方对话理解需要显式 speaker awareness 与局部交互建模，不能把 utterance 当作无主体文本。
- [Yu et al., *Speaker-Aware Discourse Parsing on Multi-Party Dialogues*, COLING 2022](https://aclanthology.org/2022.coling-1.477/)：
  speaker interaction 能改善多方 discourse dependency 建模，支持保留 speaker identity 与 Topic 局部性。
