...
| 历史 retention / permanent memory | 已落地 | token-aware projection、冻结 source digest、累计 summary、quota view、lease/fencing compaction；旧 JSONB/永久记录表已无损迁移并删除 |
...
│   │   │   └── retention.py        # compaction/permanent-memory aggregate
...
消息表取代单行 JSONB；context builder 按 sequence 分页读取，并把 summary/retention 作为显式 artifact。旧 `chat_records.messages` 数据一次性迁移后删除，不维持双写和旧 JSON API。
...
| `application/telegram/archive_utils.py` | `application/conversation/retention.py` + outbox | archive 是 conversation lifecycle，不直接 send |
...
| chat archive、history trim、summary trigger、permanent snapshot | conversation retention/summary workflow |
