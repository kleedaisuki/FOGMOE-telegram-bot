...
| Accounts / Economy | 已落地 | registration/lottery/gift/check-in 等**当前公开**金币流动使用短事务、稳定 receipt 与银行 posting；`bank` 是金额 source of truth。`identity.users` 的余额列只是由账本投影触发器维护的只读镜像，数据库守卫拒绝直接金币写入 |
...
- `bank` 是当前公开金币流动的 source of truth；`identity.users.coins` / `coins_paid` 是账本投影，数据库触发器只允许嵌套在 Bank posting 的投影更新或账户零余额初始化。0054 的不可变 Bank 账本分录保留了历史审计证据；0058 已删除旧 Assistant 预留表、其触发器和无操作赠币表。当前 Assistant 与 `/tl` 不读取或写入余额。
...
0054 的 Bank 账本迁移分录仍是历史金币变化的审计证据；0058 已删除旧 Assistant 计费表、其专用触发器/函数及无操作赠币表，运行时没有兼容 adapter 可重启该路径。`identity.users.coins` / `coins_paid` 的直接写入同样由数据库 guard 拒绝；余额只能由嵌套在 Bank posting 中的投影触发器更新。
...
| `application/banking/*` | `domain/banking` + `application/banking` + `database/banking.py` + bank Telegram adapter | 银行是当前公开金币流动的 source of truth；双钱包不可互换；申请审批、直接发行与活动奖池注资都有审计回执，余额镜像受数据库 guard 保护 |
...
- [x] `identity.users.coins` / `coins_paid` 有数据库级 direct-write guard；只允许零余额账户初始化和嵌套在 Bank posting 触发器中的投影更新。
