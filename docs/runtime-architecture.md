...
> 审计基线：2026-07-11，重构开始前的 Git `HEAD`；实现状态更新于 2026-07-12。
...
7. 不保留“旧实现兼容层”。每个迁移切片完成时同时删除旧入口、别名、锁和 facade。

### 1.1 当前迁移状态（2026-07-12）
...
| Games / RPS / RPG | 已落地 | gamble/sicbo/omikuji/RPG 用不可混用的 command/result 类型与窄 service/port；session、cooldown、库存和 receipt 持久化；OCC、稳定锁序、崩溃恢复 worker |
| Accounts / Economy | 已落地 | registration/lottery/gift/check-in/shop/stake 等使用短事务、稳定 receipt 与账本 posting；application/adapter 按 accounts/topup/rewards/community/redemption/referral/shop 直接分区 |
| Assistant Agent + Tool Effects | 已落地并接入 durable inference | provider-neutral async completion port；step checkpoint 先于 tool；读写均有 receipt；写操作与业务事实/媒体 outbox 原子提交 |
| `/tl` Translation | 已并入 durable Conversation workflow | 0..3 费用与 Turn acceptance 原子提交；专用 prompt、禁用工具；翻译输入/输出永久排除出 Assistant 历史；旧 sync task runner 已删除 |
...
| Context Window / permanent memory | 已拆分 | `context_window` 拥有 token-aware projection、checkpoint 与 lease/fencing compaction；`memory` 拥有 quota-aware records、provenance 与检索；旧 JSONB/永久记录表已无损迁移并删除 |
| 群消息投影 | 已落地 | `(group_id,message_id)` canonical 幂等投影；编辑按 `update_id` 防陈旧覆盖；Assistant/Admin 只读新投影 |
| Media picture / music | 已落地 | Picture 的请求 receipt、扣费、offer 与 photo outbox 同事务；Picture/Music 分别拥有 service、runtime、port、HTTP/DB adapter 与 Telegram handler，不再经过 Media 总门面 |
...
| 其余 Telegram feature adapters | 显式 catalog 边界 | 44 个互斥 primary 已由 durable inbox + keyed runtime 调度；部分 UI 回复仍是 Telegram at-least-once 直发，是剩余投递风险而非第二入口 |
...
- RPS 金币和 session 已持久化，但 timeout/shutdown 后的 UI 清理通知仍是 best-effort。
...
[handler_catalog.py](../src/fogmoe_bot/presentation/telegram/handler_catalog.py) 当前显式声明正式 catalog 边界的 44 个 handler；durable conversation router 独占 Assistant candidates，不再把 Assistant `reply` handler 注册到 PTB catalog：

| Handler 类型 | 数量 |
|---|---:|
| `CommandHandler` | 26 |
| `CallbackQueryHandler` | 15 |
| `MessageHandler` | 2 |
| `ChatMemberHandler` | 1 |

44 个定义都是互斥 namespace 的 primary；catalog 构造时拒绝重复 command 和 callback namespace。guard 与 observer 已由 application router 的类型化阶段承载，catalog 不再保留永不匹配的 role/group 抽象。所有定义由 durable inbox dispatcher 消费，不注册到 PTB `Application` 形成第二条更新入口。[PTB Application.add_handler](https://docs.python-telegram-bot.org/en/stable/telegram.ext.application.html#telegram.ext.Application.add_handler)

当前隐式 pipeline 是：

```text
durable ingress guards
  -> 一个 primary route（专用 durable route 或 44 项 catalog 的互斥匹配）
  -> durable observers
```
...
| `application/crypto/swap_fogmoe_solana_token.py` | global `lock` |
| `application/economy/charge_coin.py` | `code_locks`、`code_lock_mutex` |
...
| sync→async DB bridge | `accounts/service.py`、assistant memory/schedule/user tools、summary、group history 调 `db_connection.run_sync()` | worker 用 `run_coroutine_threadsafe(...).result()` 等待 event loop；形成隐藏的跨线程同步依赖 |
...
[connection.py](../src/fogmoe_bot/infrastructure/database/connection.py) 的 `insert_chat_records()` 在 transaction 中：
...
│   │   └── memory/                  # permanent-memory identity/provenance/record
...
│   └── infrastructure/
│       ├── database/
│       │   ├── db.py                # one process lifecycle, async engine only
│       │   ├── connection.py        # typed SQL + short transaction primitives
...
│       │   ├── game_operations/     # game/RPG adapters by capability
│       │   ├── media/               # picture/music persistence adapters
│       │   ├── moderation/          # group/effects/reports adapters
│       │   └── repositories/        # remaining cohesive schedule/user/verification adapters
...
| `fogmoe_dbctl` | 自身 migration/config | `fogmoe_bot` 任意包 |
...
| 用户账户 | `account:{user_id}` | 单账户命令本地有序 | account row + ledger/reservation |
| 双账户转移 | 不制造组合 actor；DB 中按 `min(user_id), max(user_id)` 固定顺序锁行 | 可避免死锁；runtime key 仅作 admission hint | 单 DB transaction |
| stake reward pool | `economy:stake-pool:{pool_id}` | pool 更新有序 | pool row/ledger |
...
| RPS 匹配队列 | `matchmaking:rps:{scope_id}` | 加入/退出等待队列有序 | durable matchmaking record |
| RPS 对局 | `game:rps:{game_id}` | 两位玩家 callback 与 timeout 有序 | game session |
| Sicbo/单人局 | `game:sicbo:{user_id}:{game_id}` | 押注/结算不可重入 | game session/ledger |
| Omikuji | `game:omikuji:{user_id}:{date}` | 每日唯一 | DB unique constraint |
| RPG 角色 | `game:rpg-character:{user_id}` | 角色 mutation 有序 | RPG tables/version |
| Crypto prediction | `crypto:prediction:{user_id}` | 创建、检查、结算有序 | prediction row/scheduled job |
| Top-up/redeem code | `economy:redemption:{code_hash}` | 本地削峰 | DB unique + conditional update |
...
消息表取代单行 JSONB；Context Window projector 按 sequence 分页读取，并把 checkpoint 作为显式 artifact。永久记忆由独立 `memory.records` 投影拥有；旧 `chat_records.messages` 数据一次性迁移后删除，不维持双写和旧 JSON API。
...
AI 费用不再“先扣完再祈祷后续成功”，而是：

```text
ReserveCoins(turn_id, amount)
→ inference activity
→ SettleReservation(turn_id) on accepted billable result
→ ReleaseReservation(turn_id) on cancellation/final inference failure
```

投递失败是否收费是产品策略，必须成为枚举/策略对象，而非异常分支。推荐默认：推理成功即结算，Telegram 最终失败不退款但保留可重投结果；媒体预处理或所有 provider 均失败则 release。相同 `turn_id` 的 reserve/settle/release 都幂等。
...
| P3 Transactional fact | 金钱、权益或审计事实 | account ledger、stake、inventory、redemption、message log、moderation policy | DB constraint + transaction；append/conditional mutation；不可依赖进程锁 |
...
| Concurrency conflict | expected version 不匹配、fencing token 过期 | reload；丢弃 stale result 或有限重算 | 条件性 |
...
| 账户 reserve/settle/release | `turn_id`/reservation ID | ledger unique + legal transition |
...
| `application/telegram/archive_utils.py` | `application/context_window` + `application/memory` | compaction checkpoint 与永久 archive 使用不同模型 |
...
| `application/economy/*` | 按 accounts/topup/rewards/community/redemption/referral/shop/staking 直接 feature module + thin Telegram handlers | 余额、code、stake、invite 靠 DB invariant/receipt/ledger，不靠 module lock 或 mega repository |
| `application/games/*` | 按 gamble/sicbo/omikuji/RPG capability 的 model/service/port + thin Telegram handlers | active game/cooldown 持久化；结果码不可跨游戏混用；无 `GamesService` |
...
| `application/media/pic.py`、`music.py` | Picture/Music 各自 application service/port/runtime + HTTP/DB/Telegram adapter | HTTP、artifact、rate-limit 和 UI 分离；无 Media 总 service/repository/handler |
| `application/crypto/*` | domain rules + application workflow + `database/crypto_operations/{chart,prediction,swap}.py` | Binance SDK 不出现在 application；无 Crypto mega repository/handler |
...
| `infrastructure/database/db.py`、`connection.py` | 保留为 infrastructure-only 的 typed async engine/SQL primitives | 已删除会话业务、JSON read-modify-write、`run_sync()` 与跨-loop 绑定；SQL 只由 infrastructure adapters 调用 |
...
| `infrastructure/assistant/tool_operations/*` | 按 memory/group/diary/schedule/social/outbound/external 实现 tool capability | catalog 是唯一 metadata source；dispatcher 只管 transaction mode 与 receipt 编排 |
...
3. `infrastructure/database/connection.py` 中的会话业务、token 计数、同步桥和 JSON read-modify-write：已删除；剩余 typed async primitives 仅供 infrastructure repository/UoW adapter 使用。
...
| permanent archive、quota view、search | Memory query/formation workflow |
| `db.py` engine registry + `connection.py` bridge | 单 event loop engine lifecycle + async UoW |
...
- 删除 `connection.py` facade、所有 sync bridge、所有 application→Telegram/SQLAlchemy/aiohttp import。
...
- [x] reservation 的 settle/release 可重放；同一 Turn 不会二次扣费或二次入池。
