...
def test_forbidden_review_and_group_request_do_not_touch_bank_state() -> None:
    """@brief 拒绝路径不会触达银行状态 / Rejected paths never touch banking state."""

    async def scenario() -> None:
        """@brief 执行群聊和越权审核场景 / Execute group-chat and unauthorized-review scenarios.
        ...
        await handler.handle(
            _update(30),
            _command("request_tokens", "5 群聊禁止申请", chat_type="supergroup"),
        )
        await handler.handle(
            _update(31),
            _command(
                "bank_review",
                "33333333-3333-4333-8333-333333333333 approve 越权测试",
                user_id=99,
            ),
        )

        assert operations.requests == []
        assert operations.reviews == []
        assert len(outbound.commands) == 2
        assert [command.payload["chat_id"] for command in outbound.commands] == [-100_42, 99]

    asyncio.run(scenario())


def test_administrator_request_is_rejected_before_notification() -> None:
    """@brief 管理员不能创建自己无法审核的申请 / An administrator cannot create a request they cannot review."""

    async def scenario() -> None:
        """@brief 执行管理员错误入口场景 / Execute the administrator wrong-entry scenario.
        ...
        await handler.handle(
            _update(32),
            _command("recharge", "5 管理员审计测试", user_id=1),
        )

        assert operations.requests == []
        assert len(outbound.commands) == 1
        assert "不能审核自己的申请" in str(outbound.commands[0].payload["text"])
