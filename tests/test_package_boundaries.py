...
def test_agent_runtime_is_application_owned_and_old_domain_package_is_removed():
    ...
    assert (llm_root / "provider_completion.py").is_file()
    assert (llm_root / "openai_codec.py").is_file()
    assert (llm_root / "anthropic_codec.py").is_file()
    assert not (llm_root / "protocol.py").exists()
    assert not (llm_root / "tool_serialization.py").exists()
    assert not (llm_root / "litellm_client.py").exists()
