...
def test_migrate_loads_package_owned_scripts_without_repository_ini(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """@brief 普通 wheel 不依赖仓库 Alembic 配置 / An ordinary wheel does not depend on repository Alembic configuration.

    @param tmp_path 不含仓库文件的启动目录 / Startup directory without repository files.
    @param monkeypatch pytest 环境替换器 / pytest environment patcher.
    @return None / None.
    """
    ...
    def fake_upgrade(config: Config, revision: str) -> None:
        """@brief 加载脚本目录而不连接数据库 / Load the script directory without connecting to a database.

        @param config 由 dbctl 生成的 Alembic 配置 / Alembic configuration generated by dbctl.
        @param revision 请求的目标 revision / Requested target revision.
        @return None / None.
        """

        script_directory = ScriptDirectory.from_config(config)
        assert script_directory.get_current_head() is not None
...
def test_migration_environment_does_not_mutate_import_paths() -> None:
    """@brief 迁移环境不再假定源码 src 根目录 / Migration environment no longer assumes a source src root.
    ...
    assert "SRC_ROOT" not in environment_source
