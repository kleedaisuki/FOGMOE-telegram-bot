...
/**
 * @brief 设置 operator socket 的帧缓冲和 deadline / Set operator-socket frame buffers and deadlines.
...
[[nodiscard]] Result<void> configure_operator_client_socket(const int descriptor) {
    ...
    if (setsockopt(descriptor, SOL_SOCKET, SO_SNDBUF, &requested_buffer, sizeof(requested_buffer)) != 0 ||
        setsockopt(descriptor, SOL_SOCKET, SO_RCVBUF, &requested_buffer, sizeof(requested_buffer)) != 0) {
        return std::unexpected(errno_error(ErrorCode::io_failure, "configure operator SOCK_SEQPACKET buffer"));
    }
    int actual_buffer{0};
    ...
    if (getsockopt(descriptor, SOL_SOCKET, SO_SNDBUF, &actual_buffer, &actual_size) != 0 ||
        ...
        return std::unexpected(make_error(ErrorCode::io_failure, "operator SOCK_SEQPACKET buffer is below protocol minimum"));
    }
    ...
    if (setsockopt(descriptor, SOL_SOCKET, SO_SNDTIMEO, &timeout, sizeof(timeout)) != 0 ||
        ...
        return std::unexpected(errno_error(ErrorCode::io_failure, "configure operator SOCK_SEQPACKET deadline"));
    }
    return {};
}

/**
 * @brief 连接并验证 root-owned operator broker process / Connect to and authenticate the root-owned operator broker process.
...
[[nodiscard]] Result<int> connect_authenticated_operator(const std::string& socket_path) {
    ...
    if (descriptor < 0) {
        return std::unexpected(errno_error(ErrorCode::io_failure, "create operator client SOCK_SEQPACKET"));
    }
...
/**
 * @brief 将无文本的 operator wire 错误转换为本地错误 / Convert a text-free operator wire error into a local error.
...
[[nodiscard]] Error map_operator_error(const operator_protocol::ErrorResponse& response) {
    switch (response.code) {
        ...
        case operator_protocol::OperatorErrorCode::protocol_violation:
            return make_error(ErrorCode::protocol_violation, "operator endpoint rejected the wire protocol");
    }
    return make_error(ErrorCode::protocol_violation, "operator endpoint returned an unknown error code");
}
 ...
 */
[[nodiscard]] Result<operator_protocol::OperatorFrame> receive_operator_response(const int descriptor) {
...
[[nodiscard]] Result<void> return_operator_error(const operator_protocol::OperatorFrame& frame) {
    if (frame.kind != operator_protocol::OperatorMessageKind::error_response) {
        return std::unexpected(make_error(ErrorCode::protocol_violation, "operator endpoint returned an unexpected frame kind"));
    }
...
OperatorGatewayClient::OperatorGatewayClient(std::string socket_path) : socket_path_(std::move(socket_path)) {}

const std::string& OperatorGatewayClient::socket_path() const noexcept {
    return socket_path_;
}
...
Result<void> OperatorGatewayClient::validate_socket_path(const std::string_view socket_path) {
    if (socket_path.empty() || socket_path.front() != '/' || socket_path.find('\0') != std::string_view::npos ||
        ...
        return std::unexpected(make_error(ErrorCode::invalid_argument, "operator socket path must be an absolute AF_UNIX endpoint"));
    }
    return {};
}

Result<domain::OperatorWorkspaceStatus> OperatorGatewayClient::status(const domain::RuntimeId& runtime) const {
    ...
    const auto request_frame = operator_protocol::encode_operator_frame(
        operator_protocol::OperatorMessageKind::status_request,
        *request_payload);
    if (!request_frame) {
    ...
    const auto close_with_error = [&](const Error& error) -> Result<domain::OperatorWorkspaceStatus> {
        close_fd(*descriptor);
        return std::unexpected(error);
    };
    if (const auto sent = operator_protocol::send_operator_frame(*descriptor, *request_frame); !sent) {
    ...
    if (response->kind != operator_protocol::OperatorMessageKind::status_response) {
        return close_with_error(make_error(ErrorCode::protocol_violation, "operator endpoint returned an unexpected status frame"));
    }
...
Result<domain::WorkspaceListing> OperatorGatewayClient::list(
    ...
    const auto request_frame = operator_protocol::encode_operator_frame(
        operator_protocol::OperatorMessageKind::list_request,
        *request_payload);
    if (!request_frame) {
        return std::unexpected(request_frame.error());
    }
    ...
    if (response->kind != operator_protocol::OperatorMessageKind::list_response) {
        return close_with_error(make_error(ErrorCode::protocol_violation, "operator endpoint returned an unexpected list frame"));
    }
