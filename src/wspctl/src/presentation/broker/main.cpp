...
int main(const int argc, char* argv[]) {
    ...
    for (int index = 1; index < argc;) {
        ...
        if (option == "--socket") {
            ...
            have_cgroup = true;
        } else if (option == "--lxcfs-root") {
            config.sandbox.lxcfs_root = value;
            have_lxcfs = true;
        ...
        } else if (option == "--tmp-size-bytes") {
            have_tmp_size = parse_unsigned(value, config.sandbox.tmp_size_bytes);
            ...
            have_project_id_min =
                parse_unsigned(value, config.sandbox.xfs_project_quota.project_id_min);
        } else if (option == "--xfs-project-id-max") {
            have_project_id_max =
                parse_unsigned(value, config.sandbox.xfs_project_quota.project_id_max);
        } else if (option == "--runtime-control-hard-bytes") {
            have_control_hard_bytes =
                parse_unsigned(value, config.sandbox.xfs_project_quota.control_hard_bytes);
        } else if (option == "--runtime-control-hard-inodes") {
            have_control_hard_inodes =
                parse_unsigned(value, config.sandbox.xfs_project_quota.control_hard_inodes);
        } else if (option == "--runtime-workspace-hard-bytes") {
            have_workspace_hard_bytes =
                parse_unsigned(value, config.sandbox.xfs_project_quota.workspace_hard_bytes);
        } else if (option == "--runtime-workspace-hard-inodes") {
            have_workspace_hard_inodes =
                parse_unsigned(value, config.sandbox.xfs_project_quota.workspace_hard_inodes);
        } else if (option == "--xfs-global-admission-bytes") {
            have_admission_bytes =
                parse_unsigned(value, config.sandbox.xfs_project_quota.global_admission_bytes);
        } else if (option == "--xfs-global-admission-inodes") {
            have_admission_inodes =
                parse_unsigned(value, config.sandbox.xfs_project_quota.global_admission_inodes);
        } else if (option == "--xfs-system-reserve-bytes") {
            have_system_reserve_bytes =
                parse_unsigned(value, config.sandbox.xfs_project_quota.system_reserve_bytes);
        } else if (option == "--xfs-system-reserve-inodes") {
            have_system_reserve_inodes =
                parse_unsigned(value, config.sandbox.xfs_project_quota.system_reserve_inodes);
        } else {
    ...
    if (!have_socket || !have_operator_socket || !have_state || !have_image_store ||
        !have_image_digest || !have_client_uid || !have_operator_uid || !have_cgroup ||
        !have_lxcfs || !have_sandbox_uid || !have_sandbox_gid || !have_memory || !have_tmp_size ||
        !have_cpu_quota || !have_cpu_period || !have_pids || !have_idle || !have_quota_backend ||
        !have_quota_mount || !have_project_id_min || !have_project_id_max ||
        !have_control_hard_bytes || !have_control_hard_inodes || !have_workspace_hard_bytes ||
        !have_workspace_hard_inodes || !have_admission_bytes || !have_admission_inodes ||
        !have_system_reserve_bytes || !have_system_reserve_inodes) {
        std::fputs("wspctld: secure configuration is mandatory\n", stderr);
        return 64;
    }
    auto broker = wspctl::Broker::create(std::move(config));
    if (!broker) {
        std::fprintf(stderr, "wspctld: fail-closed preflight rejected configuration: %s\n",
    ...
    if (!served) {
        std::fprintf(stderr, "wspctld: listener failure: %s\n", served.error().message.c_str());
        return 70;
    }
    return 0;
}
