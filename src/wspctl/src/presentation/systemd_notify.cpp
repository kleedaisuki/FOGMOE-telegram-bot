...
Result<void> notify_systemd_ready() {
    ...
    if (notification_fd < 0) {
        return std::unexpected(errno_error(ErrorCode::io_failure, "create systemd notification socket"));
    }
    ...
    const ssize_t sent = sendto(
        ...
        address_length);
    /** @brief 在 close 覆盖 errno 前保存的发送错误 / Send errno preserved before close can overwrite it. */
    const int send_error = errno;
    static_cast<void>(close(notification_fd));
    if (sent < 0) {
        errno = send_error;
        return std::unexpected(errno_error(ErrorCode::io_failure, "send systemd readiness notification"));
    }
    if (static_cast<std::size_t>(sent) != kReadyMessage.size()) {
        return std::unexpected(make_error(
            ErrorCode::io_failure,
            "systemd readiness notification was only partially sent"));
    }
    return {};
}
