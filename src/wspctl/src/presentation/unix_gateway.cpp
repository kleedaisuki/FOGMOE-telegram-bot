...
/**
 * @brief 为 client socket 设置有界 packet 与 deadline / Set bounded packets and a deadline on a
 * client socket.
 * @param fd client socket FD / Client socket FD.
 * @param deadline I/O deadline / I/O deadline.
 * @return 成功或 I/O 错误 / Success or I/O error.
 */
[[nodiscard]] Result<void> configure_client_socket(const int fd,
    ...
    if (setsockopt(fd, SOL_SOCKET, SO_SNDBUF, &requested_buffer, sizeof(requested_buffer)) != 0 ||
        ...
        return std::unexpected(
            errno_error(ErrorCode::io_failure, "configure client SOCK_SEQPACKET buffer"));
    }
    int actual_buffer = 0;
    ...
    if (getsockopt(fd, SOL_SOCKET, SO_SNDBUF, &actual_buffer, &actual_size) != 0 ||
        ...
        return std::unexpected(make_error(
            ErrorCode::io_failure, "client SOCK_SEQPACKET buffer is below protocol minimum"));
    }
    ...
    if (setsockopt(fd, SOL_SOCKET, SO_SNDTIMEO, &timeout, sizeof(timeout)) != 0 ||
        ...
        return std::unexpected(
            errno_error(ErrorCode::io_failure, "configure client SOCK_SEQPACKET deadline"));
    }
    return {};
}

/**
 * @brief 将 presentation DTO 转换为 control request / Convert a presentation DTO into a control
 * request.
...
/**
 * @brief 将 control result 转换为 presentation DTO / Convert a control result into a presentation
 * DTO.
...
/**
 * @brief 将 presentation DTO 转换为只读状态请求 / Convert a presentation DTO into a read-only
 * status request.
 * @param request presentation 状态查询 DTO / Presentation status-query DTO.
 * @return control-plane 状态查询 / Control-plane status query.
 */
[[nodiscard]] RuntimeStatusRequest
to_runtime_status_request(const ClientRuntimeStatusRequest& request) {
...
/**
 * @brief 将 control-plane 状态转换为 presentation DTO / Convert control-plane status into a
 * presentation DTO.
...
/**
 * @brief 将文件 DTO 转换为 control-plane 开始请求 / Convert a file DTO into a control-plane begin
 * request.
...
/**
 * @brief 将 presentation DTO 转换为只读 replay control request / Convert a presentation DTO into a
 * read-only replay control request.
 ...
 */
[[nodiscard]] PayloadReplayRequest
to_payload_replay_request(const ClientReplayFileRequest& request) {
...
/**
 * @brief 将 control-plane 文件收据转换为 presentation DTO / Convert a control-plane file receipt
 * into a presentation DTO.
 ...
 */
[[nodiscard]] Result<int> connect_authenticated_broker(const std::string& socket_path,
                                                       const std::chrono::milliseconds deadline) {
...
[[nodiscard]] Result<void> return_broker_error(const Frame& frame) {
    if (frame.kind != MessageKind::error) {
        return std::unexpected(
            make_error(ErrorCode::protocol_violation, "broker returned an unexpected file frame"));
    }
...
/** @brief 文件传输异常退出时的 abort/FD RAII 守卫 / Abort-and-FD RAII guard for exceptional
 * file-transfer exits. */
...
public:
     ...
     */
    PayloadTransferGuard(int& fd, std::string request_id)
    ...
    /** @brief 标记 PID 1 已持有一个未发布临时文件 / Mark that PID 1 holds an unpublished temporary
     * file. */
    void arm() noexcept { armed_ = true; }

    /** @brief 标记 publish 或 replay 已取得确定终态 / Mark that publish or replay reached a
     * deterministic terminal state. */
    void disarm() noexcept { armed_ = false; }

    /** @brief 析构时尽力 abort 未发布文件并关闭 FD / Best-effort abort an unpublished file and
    ...
    ~PayloadTransferGuard() {
        if (armed_ && fd_ >= 0) {
            try {
                ...
                // Destruction must not terminate a Python callback unwind merely because
                // best-effort cleanup allocation failed. Closing the peer still makes the broker
                // clean up state.
            }
        }
        close_fd(fd_);
...
UnixGatewayClient::UnixGatewayClient(std::string socket_path)
    : socket_path_(std::move(socket_path)) {}

const std::string& UnixGatewayClient::socket_path() const noexcept { return socket_path_; }
...
Result<void> UnixGatewayClient::validate_socket_path(const std::string_view socket_path) {
    if (socket_path.empty() || socket_path.front() != '/' ||
        ...
        return std::unexpected(make_error(ErrorCode::invalid_argument,
                                          "socket_path must be an absolute AF_UNIX pathname"));
    }
    return {};
}

Result<ClientExecutionResult>
UnixGatewayClient::execute(const ClientExecuteRequest& client_request) const {
    ...
    if (const auto configured =
            configure_client_socket(fd, request.timeout + std::chrono::seconds(5));
        !configured) {
    ...
    if (getsockopt(fd, SOL_SOCKET, SO_PEERCRED, &credentials, &credential_size) != 0 ||
        credential_size != sizeof(credentials) || credentials.uid != 0U || credentials.pid <= 0) {
        return close_with_error(make_error(ErrorCode::authentication_failed,
                                           "broker peer is not the root-owned wspctld"));
    }
    ...
    if (frame->kind != MessageKind::result) {
        return std::unexpected(
            make_error(ErrorCode::protocol_violation, "broker returned an unexpected frame"));
    }
    const auto result = decode_execution_result(frame->payload);
    if (!result || result->request_id != request.request_id) {
        return std::unexpected(
            make_error(ErrorCode::protocol_violation, "broker result identity mismatch"));
    }
    return to_client_result(*result);
}

Result<ClientRuntimeStatus>
UnixGatewayClient::status(const ClientRuntimeStatusRequest& client_request) const {
    if (const auto endpoint = validate_socket_path(socket_path_); !endpoint) {
        return std::unexpected(endpoint.error());
    }
    ...
    /** @brief 单个只读 runtime 状态查询的本机 I/O deadline / Local I/O deadline for one read-only
    ...
    if (result->handle_activation_matches != derived_activation_match) {
        return std::unexpected(
            make_error(ErrorCode::protocol_violation,
                       "broker returned an inconsistent runtime activation match"));
    }
    return to_client_runtime_status(*result);
}

Result<ClientAddFileResult> UnixGatewayClient::add_file(const ClientAddFileRequest& client_request,
    ...
    /** @brief 本机 8 MiB 流传输的单 I/O deadline / Per-I/O deadline for a local 8 MiB stream
     * transfer. */
    ...
    const auto send_payload_frame = [fd](const MessageKind kind,
    ...
    if (first_frame->kind == MessageKind::payload_result) {
        ...
        if (!replay || !replay->replayed || replay->request_id != request.request_id ||
            ...
            return close_with_error(make_error(ErrorCode::protocol_violation,
                                               "broker returned an invalid file replay"));
        }
    ...
    if (first_frame->kind != MessageKind::payload_ack) {
        return close_with_error(
            make_error(ErrorCode::protocol_violation, "broker did not acknowledge file begin"));
    }
    const auto begin_ack = decode_payload_ack(first_frame->payload);
    if (!begin_ack || begin_ack->request_id != request.request_id ||
        begin_ack->stage != PayloadAckStage::begun || begin_ack->received_bytes != 0U) {
        return close_with_error(
            make_error(ErrorCode::protocol_violation,
                       "broker returned an invalid file begin acknowledgement"));
    }
    transfer_guard.arm();
    ...
    for (;;) {
        ...
        if (const auto sent = send_payload_frame(MessageKind::payload_chunk, *encoded_chunk);
            !sent) {
            return close_with_error(sent.error());
        }
        ...
        if (response->kind != MessageKind::payload_ack) {
            return close_with_error(
                make_error(ErrorCode::protocol_violation, "broker did not acknowledge file chunk"));
        }
        ...
        if (!acknowledgement || acknowledgement->request_id != request.request_id ||
            ...
            return close_with_error(
                make_error(ErrorCode::protocol_violation,
                           "broker returned an invalid file chunk acknowledgement"));
        }
    }
    if (received_bytes != request.byte_size) {
        return close_with_error(make_error(ErrorCode::invalid_argument,
                                           "file chunk source ended before declared byte size"));
    }
    ...
    if (seal_frame->kind != MessageKind::payload_ack) {
        return close_with_error(
            make_error(ErrorCode::protocol_violation, "broker did not acknowledge file seal"));
    }
    const auto seal_ack = decode_payload_ack(seal_frame->payload);
    if (!seal_ack || seal_ack->request_id != request.request_id ||
        seal_ack->stage != PayloadAckStage::sealed ||
        seal_ack->received_bytes != request.byte_size) {
        return close_with_error(make_error(ErrorCode::protocol_violation,
                                           "broker returned an invalid file seal acknowledgement"));
    }
    const auto publish_payload = encode_payload_control_request(control);
    if (!publish_payload) {
        return close_with_error(publish_payload.error());
    }
    if (const auto sent = send_payload_frame(MessageKind::payload_publish, *publish_payload);
        !sent) {
        return close_with_error(sent.error());
    }
    ...
    if (publish_frame->kind != MessageKind::payload_result) {
        return close_with_error(
            make_error(ErrorCode::protocol_violation, "broker did not return a file receipt"));
    }
    ...
    if (!result || result->replayed || result->request_id != request.request_id ||
        ...
        return close_with_error(
            make_error(ErrorCode::protocol_violation, "broker returned an invalid file receipt"));
    }
    transfer_guard.disarm();
    return to_client_file_result(*result);
}

Result<ClientAddFileResult>
UnixGatewayClient::replay_file(const ClientReplayFileRequest& client_request) const {
    if (const auto endpoint = validate_socket_path(socket_path_); !endpoint) {
        return std::unexpected(endpoint.error());
    }
    ...
    /** @brief 单个只读 recovery lookup 的本机 I/O deadline / Local I/O deadline for one read-only
     * recovery lookup. */
    ...
    if (inbound->kind != MessageKind::payload_result) {
        return std::unexpected(make_error(ErrorCode::protocol_violation,
                                          "broker returned an unexpected file replay frame"));
    }
