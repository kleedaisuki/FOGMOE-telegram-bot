 ...
 *       Image creation belongs to a controlled build pipeline; this tool only verifies rootfs,
...
int main(const int argc, char* argv[]) {
    ...
    const unsigned int selected_modes = static_cast<unsigned int>(verify) +
                                        static_cast<unsigned int>(seal) +
                                        static_cast<unsigned int>(inspect);
    if (selected_modes != 1U) {
        std::fputs("wspctl-image: select exactly one of --verify true, --inspect true, or --seal\n",
                   stderr);
        return 64;
    }
    if (seal) {
        if (base_root.empty() || source_oci_manifest_digest.empty() || platform.empty() ||
            !images_root.empty()) {
            std::fputs("usage: wspctl-image --seal --base-root ABS "
        ...
        const auto manifest =
            wspctl::seal_image_root(base_root, platform, source_oci_manifest_digest);
        if (!manifest) {
            std::fprintf(stderr, "wspctl-image: image sealing failed: %s\n",
                         manifest.error().message.c_str());
            return 78;
        }
        std::printf("source_oci_manifest_digest=%s\nrootfs_digest=%s\n",
                    manifest->source_oci_manifest_digest.c_str(), manifest->rootfs_digest.c_str());
        return 0;
    }
    if (inspect) {
        if (base_root.empty() || !images_root.empty() || !source_oci_manifest_digest.empty() ||
            !platform.empty()) {
            std::fputs("usage: wspctl-image --inspect true --base-root ABS\n", stderr);
            return 64;
        }
        const auto manifest = wspctl::load_image_manifest(base_root);
        if (!manifest) {
            std::fprintf(stderr, "wspctl-image: sealed image inspection failed: %s\n",
                         manifest.error().message.c_str());
            return 78;
        }
        std::printf("source_oci_manifest_digest=%s\nrootfs_digest=%s\n",
    ...
    if (!manifest) {
        std::fprintf(stderr, "wspctl-image: image verification failed: %s\n",
                     manifest.error().message.c_str());
        return 78;
    }
    std::printf("source_oci_manifest_digest=%s\nrootfs_digest=%s\n",
