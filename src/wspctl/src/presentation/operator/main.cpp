...
/** @brief 非 host 安装构建不提供默认 endpoint / Non-host-install builds provide no default endpoint. */
...
void print_usage(FILE* const stream) {
    std::fputs(
        ...
        "  wspctl [--socket ABSOLUTE_SOCKET] workspace ls --runtime RUNTIME_UUID [--path /workspace]\n"
        "\n"
 ...
 */
[[nodiscard]] std::string terminal_safe_text(const std::string_view value, const bool preserve_slash) {
    constexpr std::string_view kDigits{"0123456789ABCDEF"};
    std::string rendered;
    rendered.reserve(value.size() * 3U);
    for (const unsigned char byte : value) {
        const bool safe = (byte >= static_cast<unsigned char>('a') && byte <= static_cast<unsigned char>('z')) ||
            ...
            byte == static_cast<unsigned char>('-') || (preserve_slash && byte == static_cast<unsigned char>('/'));
        if (safe) {
 ...
 */
[[nodiscard]] std::string_view persistence_name(const wspctl::domain::WorkspacePersistence persistence) noexcept {
    switch (persistence) {
 ...
 * @return 稳定 ASCII 名称 / Stable ASCII name.
 */
[[nodiscard]] std::string_view activity_name(const wspctl::domain::WorkspaceActivity activity) noexcept {
    switch (activity) {
...
/**
 * @brief 从命令行取出一个唯一 option 值 / Extract one unique option value from command-line arguments.
 ...
 */
[[nodiscard]] bool take_unique_option(
    const int argc,
    char* const argv[],
    const int begin,
...
/**
 * @brief 检查 argv 区间只由已知二值 option 组成 / Check an argv range contains only known binary options.
 * @param argc 参数数 / Argument count.
 * @param argv 参数数组 / Argument vector.
 * @param begin 开始 index / Start index.
 * @param first 允许的第一个 option / First allowed option.
 * @param second 允许的第二个 option；空字符串表示无第二项 / Second allowed option; empty means none.
 * @return 参数形状是否合法 / Whether the argument shape is valid.
 */
[[nodiscard]] bool contains_only_binary_options(
    const int argc,
    char* const argv[],
    int begin,
...
int main(const int argc, char* argv[]) {
    ...
    if (command == "workspace" && argc >= command_index + 4 && std::string_view(argv[command_index + 1]) == "ls") {
        std::string runtime_text;
