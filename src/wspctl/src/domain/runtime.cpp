...
Result<RuntimeId> RuntimeId::parse(std::string value) {
    ...
    const bool valid = value.size() == kUuidLength && std::ranges::all_of(value, [position = std::size_t{0U}](const char character) mutable {
        const bool hyphen = position == 8U || position == 13U || position == 18U || position == 23U;
        ++position;
        return hyphen ? character == '-' : is_lower_hex(character);
    });
    if (!valid) {
        return std::unexpected(make_error(ErrorCode::invalid_identity, "runtime_key must be a canonical lowercase UUID"));
    }
...
Result<ActivationId> ActivationId::parse(std::string value) {
    if (!is_safe_identifier(value)) {
        return std::unexpected(make_error(ErrorCode::invalid_identity, "activation_id must be a bounded safe identifier"));
    }
    return ActivationId(std::move(value));
}

const std::string& ActivationId::value() const noexcept {
    return value_;
}
...
Result<CommandId> CommandId::parse(std::string value) {
    if (!is_safe_identifier(value)) {
        return std::unexpected(make_error(ErrorCode::invalid_identity, "request_id must be a bounded safe identifier"));
    }
    return CommandId(std::move(value));
}

const std::string& CommandId::value() const noexcept {
    return value_;
}
...
Result<Sha256Digest> Sha256Digest::parse(std::string value) {
    /** @brief SHA-256 小写十六进制的固定字符数 / Fixed character count of lowercase hexadecimal SHA-256. */
    constexpr std::size_t kSha256HexLength{64U};
    if (value.size() != kSha256HexLength || !std::ranges::all_of(value, is_lower_hex)) {
        return std::unexpected(make_error(ErrorCode::invalid_identity, "SHA-256 digest must be 64 lowercase hexadecimal characters"));
    }
    return Sha256Digest(std::move(value));
}

const std::string& Sha256Digest::value() const noexcept {
    return value_;
}

ExecutionBudget::ExecutionBudget(const std::chrono::milliseconds wall_clock, const std::size_t output_bytes) noexcept
    : wall_clock_(wall_clock), output_bytes_(output_bytes) {}

Result<ExecutionBudget> ExecutionBudget::create(
    const std::chrono::milliseconds wall_clock,
    const std::size_t output_bytes) {
    if (wall_clock.count() <= 0 || output_bytes == 0U) {
        return std::unexpected(make_error(ErrorCode::invalid_budget, "execution wall-clock and output budgets must both be non-zero"));
    }
    return ExecutionBudget(wall_clock, output_bytes);
}

std::chrono::milliseconds ExecutionBudget::wall_clock() const noexcept {
    return wall_clock_;
}

std::size_t ExecutionBudget::output_bytes() const noexcept {
...
const RuntimeId& Runtime::id() const noexcept {
    return id_;
}
...
Result<void> Runtime::mark_ready(const ActivationId& activation) {
    if (const auto valid = require_active(RuntimeState::activating, activation, "mark ready"); !valid) {
...
Result<void> Runtime::begin_execution(const ActivationId& activation) {
    if (const auto valid = require_active(RuntimeState::ready, activation, "begin execution"); !valid) {
        return std::unexpected(valid.error());
    }
...
Result<void> Runtime::finish_execution(const ActivationId& activation) {
    if (const auto valid = require_active(RuntimeState::executing, activation, "finish execution"); !valid) {
        return std::unexpected(valid.error());
    }
    state_ = RuntimeState::ready;
    return {};
...
Result<void> Runtime::begin_retirement(const ActivationId& activation) {
    if (const auto valid = require_active(RuntimeState::ready, activation, "begin retirement"); !valid) {
        return std::unexpected(valid.error());
    }
...
Result<void> Runtime::finish_retirement(const ActivationId& activation) {
    if (const auto valid = require_active(RuntimeState::retiring, activation, "finish retirement"); !valid) {
        return std::unexpected(valid.error());
    }
...
Result<void> Runtime::require_active(
    ...
    if (!active_activation_.has_value() || *active_activation_ != activation) {
        return std::unexpected(make_error(ErrorCode::activation_mismatch, "activation does not own runtime for " + std::string(operation)));
    }
    return {};
}
