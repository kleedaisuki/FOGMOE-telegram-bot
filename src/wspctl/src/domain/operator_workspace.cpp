...
/**
 * @brief 将一个大写十六进制字符转成 nibble / Convert one uppercase hexadecimal character into a
 * nibble.
...
/**
 * @brief 判断安全显示名是否严格为 percent encoding / Check whether a safe display name is strict
 * percent encoding.
...
/**
 * @brief 判断 workspace persistence 是否为领域词表成员 / Check whether a workspace-persistence
 * value belongs to the domain vocabulary.
...
/**
 * @brief 判断 workspace activity 是否为领域词表成员 / Check whether a workspace-activity value
 * belongs to the domain vocabulary.
...
WorkspaceQuotaUsage::WorkspaceQuotaUsage(const std::uint64_t used_bytes,
                                         const std::uint64_t hard_bytes,
                                         const std::uint64_t used_inodes,
                                         const std::uint64_t hard_inodes) noexcept
    : used_bytes_(used_bytes), hard_bytes_(hard_bytes), used_inodes_(used_inodes),
      hard_inodes_(hard_inodes) {}

Result<WorkspaceQuotaUsage> WorkspaceQuotaUsage::create(const std::uint64_t used_bytes,
                                                        const std::uint64_t hard_bytes,
                                                        const std::uint64_t used_inodes,
...
std::uint64_t WorkspaceQuotaUsage::used_bytes() const noexcept { return used_bytes_; }

std::uint64_t WorkspaceQuotaUsage::hard_bytes() const noexcept { return hard_bytes_; }

std::uint64_t WorkspaceQuotaUsage::used_inodes() const noexcept { return used_inodes_; }

std::uint64_t WorkspaceQuotaUsage::hard_inodes() const noexcept { return hard_inodes_; }

OperatorWorkspaceStatus::OperatorWorkspaceStatus(RuntimeId runtime,
                                                 const WorkspacePersistence persistence,
                                                 const WorkspaceActivity activity,
                                                 std::optional<WorkspaceQuotaUsage> quota) noexcept
    : runtime_(std::move(runtime)), persistence_(persistence), activity_(activity),
      quota_(std::move(quota)) {}

Result<OperatorWorkspaceStatus>
OperatorWorkspaceStatus::create(RuntimeId runtime, const WorkspacePersistence persistence,
                                const WorkspaceActivity activity,
...
const RuntimeId& OperatorWorkspaceStatus::runtime() const noexcept { return runtime_; }

WorkspacePersistence OperatorWorkspaceStatus::persistence() const noexcept { return persistence_; }

WorkspaceActivity OperatorWorkspaceStatus::activity() const noexcept { return activity_; }
...
Result<OperatorWorkspacePath> OperatorWorkspacePath::parse(std::string value) {
    ...
    while (start < value.size()) {
        ...
        const std::string_view component(value.data() + static_cast<std::ptrdiff_t>(start),
                                         (end == std::string::npos ? value.size() : end) - start);
        if (component.empty() || component == "." || component == ".." ||
            component.size() > kMaximumFilenameBytes) {
            return std::unexpected(
                make_error(ErrorCode::invalid_identity,
...
const std::string& OperatorWorkspacePath::value() const noexcept { return value_; }
...
WorkspaceEntry::WorkspaceEntry(std::string encoded_name, const WorkspaceEntryKind kind,
                               const std::uint64_t size_bytes) noexcept
    : encoded_name_(std::move(encoded_name)), kind_(kind), size_bytes_(size_bytes) {}

Result<WorkspaceEntry> WorkspaceEntry::create(std::string encoded_name,
    ...
    if (!is_strict_encoded_name(encoded_name) || encoded_name == "." || encoded_name == "..") {
        return std::unexpected(
            make_error(ErrorCode::invalid_identity,
                       "operator workspace entry name is not safe percent encoding"));
    }
    if (kind != WorkspaceEntryKind::regular_file && kind != WorkspaceEntryKind::directory &&
        kind != WorkspaceEntryKind::symbolic_link) {
        return std::unexpected(
            make_error(ErrorCode::invalid_identity, "operator workspace entry kind is invalid"));
    }
    if (kind != WorkspaceEntryKind::regular_file && size_bytes != 0U) {
        return std::unexpected(make_error(ErrorCode::invalid_identity,
                                          "only regular workspace files may expose a byte size"));
    }
    return WorkspaceEntry(std::move(encoded_name), kind, size_bytes);
}

const std::string& WorkspaceEntry::encoded_name() const noexcept { return encoded_name_; }

WorkspaceEntryKind WorkspaceEntry::kind() const noexcept { return kind_; }
...
Result<std::string> encode_workspace_entry_name(const std::string_view raw_name) {
    if (raw_name.empty() || raw_name.size() > kMaximumFilenameBytes || raw_name == "." ||
        ...
        return std::unexpected(
            make_error(ErrorCode::invalid_identity, "raw workspace entry name is unsafe"));
    }
    std::string encoded;
