...
RuntimeStatusQuery::RuntimeStatusQuery(
...
const domain::RuntimeId& RuntimeStatusQuery::runtime() const noexcept {
    return runtime_;
}
...
RuntimeStatus::RuntimeStatus(
    ...
    : snapshot_(std::move(snapshot)),
      handle_activation_matches_(handle_activation_matches),
      supervisor_alive_(supervisor_alive),
      idle_for_(std::move(idle_for)),
      idle_ttl_(idle_ttl),
      borrowed_dispatches_(borrowed_dispatches),
      cleanup_pending_(cleanup_pending) {}
...
domain::Result<RuntimeStatus> RuntimeStatus::create(
    ...
    if (idle_ttl.count() <= 0) {
        return std::unexpected(domain::make_error(
            domain::ErrorCode::invalid_budget,
    ...
    if (supervisor_alive &&
        (snapshot.state() == domain::RuntimeState::dormant || snapshot.state() == domain::RuntimeState::failed)) {
        return std::unexpected(domain::make_error(
            domain::ErrorCode::illegal_transition,
            "a dormant or failed runtime cannot expose a healthy live supervisor"));
    }
    const bool handle_activation_matches = snapshot.active_activation().has_value() &&
...
const domain::RuntimeSnapshot& RuntimeStatus::snapshot() const noexcept {
    return snapshot_;
}
...
bool RuntimeStatus::supervisor_alive() const noexcept {
    return supervisor_alive_;
}
...
std::chrono::milliseconds RuntimeStatus::idle_ttl() const noexcept {
    return idle_ttl_;
}

std::uint64_t RuntimeStatus::borrowed_dispatches() const noexcept {
    return borrowed_dispatches_;
}

bool RuntimeStatus::cleanup_pending() const noexcept {
    return cleanup_pending_;
}

domain::Result<RuntimeStatus> RuntimeStatusService::inspect(
    const RuntimeStatusQuery& query,
