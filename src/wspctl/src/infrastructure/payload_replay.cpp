...
/**
 * @brief replay object 验证期间的最小 FD RAII owner / Minimal FD RAII owner for replay-object verification.
 */
...
public:
     ...
     */
    ReplayOwnedFd(ReplayOwnedFd&& other) noexcept : descriptor_(std::exchange(other.descriptor_, -1)) {}
 ...
 */
[[nodiscard]] std::string render_sha256_hex(const std::array<unsigned char, SHA256_DIGEST_LENGTH>& digest) {
...
Result<PayloadResult> resolve_payload_replay_receipt(
    ...
    if (!existing) {
        if (existing.error().code == ErrorCode::not_found) {
            return std::unexpected(make_error(ErrorCode::not_found, "no durable file ingress receipt exists for replay"));
        }
    ...
    if (!record.payload_result.has_value()) {
        return std::unexpected(make_error(ErrorCode::journal_conflict, "completed file ingress journal has no receipt"));
    }
    /** @brief completed receipt copied before replay marking / Completed receipt copied before replay marking. */
...
Result<void> verify_replayable_payload_object(
    const RuntimeQuotaBinding& binding,
    const PayloadReplayRequest& request,
    const PayloadResult& receipt) {
    /** @brief canonical in-workspace payload path / Canonical in-workspace payload path. */
    const std::string expected_path = "/workspace/uploads/" + request.opaque_id + "/payload";
    if (!receipt.replayed || receipt.request_id != request.request_id || receipt.path != expected_path ||
        receipt.byte_size != request.byte_size || receipt.sha256 != request.sha256) {
        return std::unexpected(make_error(ErrorCode::journal_conflict, "replay receipt changed before persistent object verification"));
    }
    /** @brief persistent workspace upper root / Persistent workspace upper root. */
    ReplayOwnedFd upper_fd(open(
        (binding.workspace_dir / "upper").c_str(),
        O_RDONLY | O_DIRECTORY | O_CLOEXEC | O_NOFOLLOW));
    if (upper_fd.get() < 0) {
        return std::unexpected(make_error(ErrorCode::invocation_in_doubt, "completed file receipt has no readable persistent workspace upper root"));
    }
    ...
    if (fstat(upper_fd.get(), &upper_metadata) != 0 || !S_ISDIR(upper_metadata.st_mode)) {
        return std::unexpected(make_error(ErrorCode::invocation_in_doubt, "persistent workspace upper root changed during replay verification"));
    }
    /** @brief uploads directory FD / Uploads-directory FD. */
    ReplayOwnedFd uploads_fd(openat(
        upper_fd.get(),
        "uploads",
        O_RDONLY | O_DIRECTORY | O_CLOEXEC | O_NOFOLLOW));
    if (uploads_fd.get() < 0) {
        return std::unexpected(make_error(ErrorCode::invocation_in_doubt, "completed file receipt payload directory is missing"));
    }
    ...
    if (fstat(uploads_fd.get(), &uploads_metadata) != 0 || !S_ISDIR(uploads_metadata.st_mode) ||
        ...
        return std::unexpected(make_error(ErrorCode::invocation_in_doubt, "completed file receipt uploads directory is not recoverable"));
    }
    /** @brief opaque payload directory FD / Opaque payload-directory FD. */
    ReplayOwnedFd payload_directory_fd(openat(
        uploads_fd.get(),
        request.opaque_id.c_str(),
        O_RDONLY | O_DIRECTORY | O_CLOEXEC | O_NOFOLLOW));
    if (payload_directory_fd.get() < 0) {
        return std::unexpected(make_error(ErrorCode::invocation_in_doubt, "completed file receipt opaque payload directory is missing"));
    }
    ...
    if (fstat(payload_directory_fd.get(), &payload_directory_metadata) != 0 ||
        !S_ISDIR(payload_directory_metadata.st_mode) || payload_directory_metadata.st_dev != upper_metadata.st_dev) {
        return std::unexpected(make_error(ErrorCode::invocation_in_doubt, "completed file receipt opaque payload directory is not recoverable"));
    }
    /** @brief no-follow payload file FD / No-follow payload-file FD. */
    ReplayOwnedFd payload_fd(openat(
        payload_directory_fd.get(),
        "payload",
        O_RDONLY | O_CLOEXEC | O_NOFOLLOW));
    if (payload_fd.get() < 0) {
        return std::unexpected(make_error(ErrorCode::invocation_in_doubt, "completed file receipt payload object is missing"));
    }
    ...
    if (fstat(payload_fd.get(), &before_digest) != 0 || !S_ISREG(before_digest.st_mode) ||
        ...
        return std::unexpected(make_error(ErrorCode::invocation_in_doubt, "completed file receipt payload object does not match its persisted size"));
    }
    /** @brief streaming SHA-256 context / Streaming SHA-256 context. */
    std::unique_ptr<EVP_MD_CTX, decltype(&EVP_MD_CTX_free)> digest_context(EVP_MD_CTX_new(), &EVP_MD_CTX_free);
    if (!digest_context || EVP_DigestInit_ex(digest_context.get(), EVP_sha256(), nullptr) != 1) {
        return std::unexpected(make_error(ErrorCode::io_failure, "initialize persistent payload SHA-256 verification"));
    }
    /** @brief bounded streaming read buffer / Bounded streaming read buffer. */
    std::array<std::byte, 64U * 1024U> buffer{};
    /** @brief total bytes read through the opened object FD / Total bytes read through the opened object FD. */
    ...
    for (;;) {
        ...
        if (count < 0) {
            ...
            return std::unexpected(make_error(ErrorCode::invocation_in_doubt, "cannot prove the completed payload object is readable"));
        }
        if (count == 0) {
            break;
        }
        ...
        if (chunk_size > request.byte_size - read_bytes ||
            ...
            return std::unexpected(make_error(ErrorCode::invocation_in_doubt, "completed payload object changed while being verified"));
        }
    ...
    if (read_bytes != request.byte_size ||
        EVP_DigestFinal_ex(digest_context.get(), digest.data(), &digest_size) != 1 || digest_size != digest.size()) {
        return std::unexpected(make_error(ErrorCode::invocation_in_doubt, "completed payload object size or digest finalization is indeterminate"));
    }
    /** @brief payload metadata after digest / Payload metadata after digest. */
    struct stat after_digest {};
    if (fstat(payload_fd.get(), &after_digest) != 0 || after_digest.st_dev != before_digest.st_dev ||
        after_digest.st_ino != before_digest.st_ino || after_digest.st_size != before_digest.st_size ||
        ...
        after_digest.st_ctim.tv_nsec != before_digest.st_ctim.tv_nsec || render_sha256_hex(digest) != request.sha256) {
        return std::unexpected(make_error(ErrorCode::invocation_in_doubt, "completed payload object changed or no longer matches its durable digest"));
    }
    return {};
}
