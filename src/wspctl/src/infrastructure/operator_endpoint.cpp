...
/**
 * @brief 验证一个 operator endpoint path 的 wire 可表示性 / Validate wire representability of an
 * operator endpoint path.
    ...
    if (value.empty() || !path.is_absolute() || path.filename().empty() ||
        ...
        return std::unexpected(
            make_error(ErrorCode::invalid_argument,
                       "operator endpoint path is not a valid absolute AF_UNIX path"));
    }
    return {};
}

/**
 * @brief 判断一个规范目录是否包含另一个目录 / Check whether one normalized directory contains
 * another.
 ...
 */
[[nodiscard]] bool contains_directory(const std::filesystem::path& ancestor,
...
Result<void> validate_operator_endpoint_separation(const std::filesystem::path& bot_socket,
                                                   const uid_t bot_uid,
    ...
    if (bot_uid == 0U) {
        return std::unexpected(
            make_error(ErrorCode::invalid_argument, "Bot endpoint UID must not be root"));
    }
    if (bot_uid == operator_uid) {
        return std::unexpected(
            make_error(ErrorCode::permission_denied,
                       "operator endpoint UID must differ from the Bot endpoint UID"));
    }
    if (bot_socket.lexically_normal() == operator_socket.lexically_normal()) {
        return std::unexpected(
            make_error(ErrorCode::invalid_argument,
                       "operator and Bot endpoints must use different socket paths"));
    }
    ...
    if (contains_directory(bot_parent, operator_parent) ||
        contains_directory(operator_parent, bot_parent)) {
        return std::unexpected(
            make_error(ErrorCode::invalid_argument,
