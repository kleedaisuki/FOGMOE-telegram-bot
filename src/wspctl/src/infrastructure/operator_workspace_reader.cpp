...
/**
 * @brief 以 openat2 打开一个不跟随符号链接的子目录 / Open a child directory without following
 * symbolic links via openat2.
 ...
 */
[[nodiscard]] Result<OwnedFileDescriptor> open_directory_beneath(const int parent_fd,
    ...
    if (parent_fd < 0 || component.empty() || component == "." || component == ".." ||
        ...
        return std::unexpected(
            make_error(ErrorCode::invalid_argument, "invalid operator workspace path component"));
    }
#ifdef SYS_openat2
    ...
    how.flags =
        static_cast<std::uint64_t>(O_RDONLY | O_DIRECTORY | O_CLOEXEC | O_NOFOLLOW | O_NOATIME);
    ...
    if (descriptor < 0) {
        /** @brief 保留 openat2 errno，避免 domain not-found 被安全映射吞掉 / Saved openat2 errno so
         * a domain not-found is not swallowed by the security mapping. */
        const int saved_errno = errno;
        if (saved_errno == ENOENT || saved_errno == ENOTDIR) {
            return std::unexpected(
                make_error(ErrorCode::not_found, "operator workspace directory was not found"));
        }
        errno = saved_errno;
        return std::unexpected(
            errno_error(ErrorCode::permission_denied, "openat2 operator workspace directory"));
    }
    ...
    if (fstat(descriptor, &metadata) != 0 || !S_ISDIR(metadata.st_mode)) {
        ...
        errno = saved_errno;
        return std::unexpected(make_error(ErrorCode::permission_denied,
                                          "operator workspace component is not a directory"));
    }
...
/**
 * @brief 将不跟随链接的 stat mode 转为 allowlisted 节点类型 / Convert an lstat mode into an
 * allowlisted node kind.
 * @param mode `fstatat(..., AT_SYMLINK_NOFOLLOW)` 返回的 mode / Mode returned by `fstatat(...,
 * AT_SYMLINK_NOFOLLOW)`.
 * @return allowlisted 节点类型；特殊文件为空 / Allowlisted node kind; empty for special files.
 */
[[nodiscard]] std::optional<domain::WorkspaceEntryKind>
entry_kind_from_mode(const mode_t mode) noexcept {
    if (S_ISREG(mode)) {
...
OperatorWorkspaceReader::list(const RuntimeQuotaBinding& binding,
                              const domain::OperatorWorkspacePath& path) const {
    /** @brief 已验证 upper root 的 host path；永不拼接用户 path / Verified upper-root host path; no
     * user path is ever appended. */
    ...
    if (directory_fd.get() < 0) {
        return std::unexpected(
            errno_error(ErrorCode::io_failure, "open verified operator workspace upper directory"));
    }
    struct stat root_metadata {};
    if (fstat(directory_fd.get(), &root_metadata) != 0 || !S_ISDIR(root_metadata.st_mode)) {
        return std::unexpected(make_error(
            ErrorCode::io_failure, "verified operator workspace upper root is not a directory"));
    }
    ...
    /** @brief `fdopendir` 专用 FD，避免接管主 directory FD / Dedicated FD for fdopendir, avoiding
     * transfer of the primary directory FD. */
    const int scan_fd = dup(directory_fd.get());
    if (scan_fd < 0) {
        return std::unexpected(
            errno_error(ErrorCode::io_failure, "duplicate operator workspace directory FD"));
    }
    DIR* const directory = fdopendir(scan_fd);
    if (directory == nullptr) {
        static_cast<void>(close(scan_fd));
        return std::unexpected(
            errno_error(ErrorCode::io_failure, "fdopendir operator workspace directory"));
    }
    ...
    for (;;) {
        ...
        /** @brief 从内核 NUL-terminated dirent 取得的原始名称 / Raw name obtained from the kernel
         * NUL-terminated dirent. */
        ...
        if (fstatat(directory_fd.get(), entry->d_name, &metadata, AT_SYMLINK_NOFOLLOW) != 0) {
            ...
            errno = saved_errno;
            return std::unexpected(
                errno_error(ErrorCode::io_failure, "inspect operator workspace directory entry"));
        }
        const std::optional<domain::WorkspaceEntryKind> kind =
            entry_kind_from_mode(metadata.st_mode);
        if (!kind.has_value()) {
            continue;
        }
        ...
        if (!encoded_name) {
            static_cast<void>(closedir(directory));
            return std::unexpected(make_error(
                ErrorCode::io_failure, "operator workspace directory entry name is invalid"));
        }
        /** @brief 仅普通文件可以报告逻辑长度 / Only regular files may report a logical length. */
        const std::uint64_t size_bytes =
            *kind == domain::WorkspaceEntryKind::regular_file && metadata.st_size >= 0
                ? static_cast<std::uint64_t>(metadata.st_size)
                : 0U;
        const auto workspace_entry =
            domain::WorkspaceEntry::create(*encoded_name, *kind, size_bytes);
        if (!workspace_entry) {
            static_cast<void>(closedir(directory));
            return std::unexpected(
                make_error(ErrorCode::io_failure,
                           "operator workspace directory entry violates domain policy"));
        }
    ...
    if (errno != 0) {
        ...
        errno = saved_errno;
        return std::unexpected(
            errno_error(ErrorCode::io_failure, "enumerate operator workspace directory"));
    }
