...
struct SandboxConfig final {
    /** @brief 仅 broker 管理的 content-addressed image store / Content-addressed image store
     * managed only by broker operations. */
    ...
    /** @brief journal、upperdir 与 runtime 元数据根 / Root for journal, upperdirs, and runtime
     * metadata. */
    std::filesystem::path state_root;
    /** @brief 唯一允许的 XFS project-quota 容量边界 / Sole permitted XFS project-quota capacity
     * boundary. */
    XfsProjectQuotaConfig xfs_project_quota;
    /** @brief systemd Delegate=yes 委派的 cgroup v2 根 / cgroup v2 root delegated by systemd
     * Delegate=yes. */
    std::filesystem::path cgroup_root;
    /** @brief 专用 LXCFS 的 cgroup-aware procfs 数据源根 / Root of the dedicated LXCFS cgroup-aware
     * procfs source. */
    std::filesystem::path lxcfs_root;
    /** @brief runtime cgroup 的 memory.max（字节） / runtime cgroup memory.max in bytes. */
    std::uint64_t memory_max_bytes{4ULL * 1024ULL * 1024ULL * 1024ULL};
    /** @brief runtime cgroup 的 memory.high（0 表示与 memory.max 相同） / runtime cgroup
     * memory.high (0 means memory.max). */
    std::uint64_t memory_high_bytes{};
    /** @brief runtime cgroup 的 memory.swap.max（字节） / runtime cgroup memory.swap.max in bytes.
     */
    std::uint64_t memory_swap_max_bytes{2ULL * 1024ULL * 1024ULL * 1024ULL};
    /** @brief runtime 私有 /tmp tmpfs 的字节上限 / Byte limit of the runtime-private /tmp tmpfs. */
    std::uint64_t tmp_size_bytes{1ULL * 1024ULL * 1024ULL * 1024ULL};
    /** @brief runtime cgroup 的 cpu.max 配额微秒 / runtime cgroup cpu.max quota in microseconds. */
    std::uint64_t cpu_max_quota_us{200'000U};
    /** @brief runtime cgroup 的 cpu.max period 微秒 / runtime cgroup cpu.max period in
     * microseconds. */
    std::uint32_t cpu_max_period_us{100'000U};
    /** @brief runtime cgroup 的 pids.max / runtime cgroup pids.max. */
    std::uint32_t pids_max{128U};
    /** @brief runtime cgroup 的 io.weight；0 表示 host 不支持时禁用 / Runtime cgroup io.weight;
     * zero disables it when unsupported by the host. */
...
/**
 * @brief 从强类型 identity 派生唯一 rootfs 路径 / Derive the sole rootfs path from a typed
 * identity.
...
struct TaskLayer final {
    ...
    /** @brief 每次 mount 新建且为空的 OverlayFS workdir / Fresh and empty OverlayFS workdir per
     * mount. */
...
/**
 * @brief broker 传给 PID 1 的 task cgroup 控制 FD / Task-cgroup control FDs passed from broker to
 * PID 1.
 */
...
/**
 * @brief 校验目录自身与生产祖先链的所有权 / Validate directory ownership and its production
 * ancestor chain.
 * @param path 已存在的绝对目录 / Existing absolute directory.
 * @param allow_insecure_dev_root 是否只放宽非终点祖先 / Whether to relax only non-terminal
 * ancestors.
 ...
 */
[[nodiscard]] Result<void> validate_secure_directory_ancestry(const std::filesystem::path& path,
 ...
 *       Missing CAP_SYS_ADMIN, cgroup delegation, image attestation, or kernel capability always
 * fails.
 */
[[nodiscard]] Result<void> preflight_sandbox(const SandboxConfig& config);

/**
 * @brief 将 broker 移入 manager leaf 后启用 delegated controllers / Move broker to a manager leaf
 * then enable delegated controllers.
 * @param config sandbox 配置 / Sandbox configuration.
 * @return 成功或拒绝 / Success or rejection.
 * @note 这样 cgroup_root 内部节点没有进程，cpu/memory/pids controller 可以沿树启用。
 *       This leaves cgroup_root internal nodes process-free so cpu/memory/pids controllers can be
 * enabled down the tree.
 */
[[nodiscard]] Result<void> prepare_broker_cgroup(const SandboxConfig& config);

/**
 * @brief 为 activation 创建 quota-verified workspace OverlayFS 挂载目录 / Create quota-verified
 * workspace OverlayFS mount directories for an activation.
 * @param config sandbox 配置 / Sandbox configuration.
 * @param quota 已 preflight 的 XFS quota 服务 / Preflighted XFS quota service.
 * @param activation_lease 为该 runtime 持有的 activation 排他租约 / Exclusive activation lease held
 * for this runtime.
 * @param activation_id 激活标识 / Activation ID.
 * @return 不含未经哈希用户路径的层路径 / Layer paths without unhashed user paths.
 * @note upperdir 按 runtime 持久化，因此新 activation 可恢复 workspace；workdir
 * 每次创建且必须为空。 The upperdir persists per runtime so a new activation recovers its
 * workspace; workdir is fresh and empty each time.
 */
[[nodiscard]] Result<TaskLayer> prepare_task_layer(const SandboxConfig& config,
                                                   const XfsProjectQuota& quota,
                                                   const RuntimeActivationLease& activation_lease,
                                                   const std::string& activation_id);

/**
 * @brief 仅删除一个已退出 activation 的 transient mount staging / Remove transient mount staging
 * for one exited activation only.
 * @param config sandbox 配置 / Sandbox configuration.
 * @param quota 已 preflight 的 XFS quota 服务 / Preflighted XFS quota service.
 * @param activation_lease 为该 runtime 持有的 activation 排他租约 / Exclusive activation lease held
 * for this runtime.
 ...
 */
[[nodiscard]] Result<void> cleanup_task_layer(const SandboxConfig& config,
                                              const XfsProjectQuota& quota,
                                              const RuntimeActivationLease& activation_lease,
                                              const TaskLayer& layer);

/**
 * @brief 在 cgroup task-free 证明之后回收 crash 遗留的 transient staging / Reclaim crash-left
 * transient staging after a cgroup task-free proof.
 * @param config sandbox 配置 / Sandbox configuration.
 * @param quota 已 preflight 的 XFS quota 服务 / Preflighted XFS quota service.
 * @param activation_lease 为该 runtime 持有的 activation 排他租约 / Exclusive activation lease held
 * for this runtime.
 ...
 */
[[nodiscard]] Result<void> reclaim_dead_task_layers(const SandboxConfig& config,
                                                    const XfsProjectQuota& quota,
                                                    const RuntimeActivationLease& activation_lease,
                                                    const std::string& runtime_key);

/**
 * @brief 新 PID/mount namespace 中建立只读 lower 与 OverlayFS / Establish readonly lower and
 * OverlayFS in new PID/mount namespace.
 * @param config sandbox 配置 / Sandbox configuration.
 ...
 */
[[nodiscard]] Result<void> setup_runtime_mounts(const SandboxConfig& config,
                                                const TaskLayer& layer);

/**
 * @brief 创建 runtime 的 supervisor/task cgroup 分层并打开控制 FD / Create runtime supervisor/task
 * cgroup hierarchy and open control FDs.
 * @param config sandbox 配置 / Sandbox configuration.
 * @param runtime_key runtime 标识 / Runtime key.
 * @return launcher/PID 1 使用的 supervisor/task cgroup FD / Supervisor/task cgroup FDs used by the
 * launcher/PID 1.
 */
[[nodiscard]] Result<TaskCgroupControl> prepare_runtime_cgroup(const SandboxConfig& config,
                                                               const std::string& runtime_key);
 ...
 * @return 成功或拒绝 / Success or rejection.
 */
[[nodiscard]] Result<void> kill_runtime_cgroup(const SandboxConfig& config,
                                               const std::string& runtime_key);

/**
 * @brief 等待 runtime cgroup 不再包含任何进程 / Wait until a runtime cgroup has no remaining
 * processes.
 * @param config sandbox 配置 / Sandbox configuration.
 * @param runtime_key runtime 标识 / Runtime key.
 * @return cgroup.events 报告 populated 0，或 fail-closed 错误 / populated 0 from cgroup.events, or
 * a fail-closed error.
 * @note 调用方在成功前不得复用或删除该 runtime cgroup。
 *       Callers must not reuse or delete the runtime cgroup before success.
 */
[[nodiscard]] Result<void> wait_runtime_cgroup_empty(const SandboxConfig& config,
                                                     const std::string& runtime_key);

/**
 * @brief 丢弃 capability、设 no_new_privs 并加载 seccomp / Drop capabilities, set no_new_privs, and
 * load seccomp.
...
/**
 * @brief 将 runtime PID 1 缩减为最小 supervisor 权限 / Reduce runtime PID 1 to minimal supervisor
 * privileges.
 * @return 成功或 fail-closed 错误 / Success or fail-closed error.
