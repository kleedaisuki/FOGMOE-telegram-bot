...
/**
 * @brief 基于 verified quota binding 的 operator workspace 目录读取器 / Operator workspace-directory reader based on a verified quota binding.
 *
...
public:
     ...
     * @param binding 已由 quota 服务 read-back 验证的 runtime binding / Runtime binding read-back verified by the quota service.
     * @param path 已验证 `/workspace` 逻辑路径 / Validated `/workspace` logical path.
     * @return 排序、有界、安全编码的目录列举或错误 / Sorted bounded safely encoded directory listing or an error.
     * @note 内层分量以 `openat2(RESOLVE_BENEATH|RESOLVE_NO_SYMLINKS|RESOLVE_NO_MAGICLINKS|RESOLVE_NO_XDEV)`
     *       fail closed；内核不支持时不会退化到字符串路径解析。/ Inner components use
     ...
     */
    [[nodiscard]] Result<domain::WorkspaceListing> list(
