 ...
 * The port describes effects required by the application layer without exposing namespace, cgroup, socket, or filesystem implementations.
 */
...
public:
    ...
    /**
     * @brief 建立指定 activation 的 RuntimeProcess / Establish the RuntimeProcess for an activation.
     ...
     */
    [[nodiscard]] virtual domain::Result<void> establish(
     ...
     * @return 成功或已归一化的领域错误 / Success or a normalized domain error.
     */
    [[nodiscard]] virtual domain::Result<void> retire(
        const domain::RuntimeId& runtime,
        const domain::ActivationId& activation) = 0;
};
 ...
 * This service is the transaction orchestration point between the domain state machine and external effects.
 * When the port fails the aggregate becomes failed, so an unproven RuntimeProcess is never returned as ready.
 */
...
public:
     ...
     */
    [[nodiscard]] domain::Result<void> activate(
    ...
    /**
     * @brief 在执行或协议失败后强制回收 activation / Force cleanup of an activation after execution or protocol failure.
     ...
     *       This use case intentionally does not require ready state: a failed aggregate cannot accept work but must still reap a real Process.
     */
    [[nodiscard]] domain::Result<void> abort(
        domain::Runtime& runtime,
        const domain::ActivationId& activation,
        RuntimeActivationPort& port) const;
};
