...
public:
    /**
     * @brief 以独立 operator socket 构造 client / Construct a client with an independent operator socket.
     ...
     */
    [[nodiscard]] Result<domain::OperatorWorkspaceStatus> status(const domain::RuntimeId& runtime) const;

    /**
     * @brief 列举一层持久 OverlayFS upper workspace / List one persistent OverlayFS-upper workspace level.
     * @param runtime 已验证 runtime 标识 / Validated runtime identity.
     * @param path 已验证 `/workspace` 逻辑路径 / Validated `/workspace` logical path.
     * @return 有界安全目录列举或 operator 错误 / Bounded safe directory listing or an operator error.
...
private:
    /** @brief 独立 operator UNIX socket 的绝对路径 / Absolute independent operator UNIX socket path. */
