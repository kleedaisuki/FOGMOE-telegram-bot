 ...
 *
 * 这是 presentation 边界的未特权请求形状；gateway 在写入 wire 前将其转换为受验证的 control
 * request。 This is the unprivileged request shape at the presentation boundary; the gateway
 * converts it to a validated control request before writing the wire.
 */
...
/** @brief Bot 到 Unix gateway 的无副作用状态查询 DTO / Side-effect-free status-query DTO from the
 * Bot to the Unix gateway. */
...
/**
 * @brief Unix gateway 返回给 Bot 的 allowlisted 运行态 DTO / Allowlisted operating-status DTO
 * returned by the Unix gateway.
 *
...
/**
 * @brief presentation 层的单消费文件 chunk 源 / Single-consumption file-chunk source at the
 * presentation layer.
 *
...
struct ClientAddFileRequest final {
    ...
    /** @brief 调用方语义 SHA-256 / Caller semantic SHA-256. */
    std::string request_hash;
    /** @brief 可信上层产生的受限 opaque 目录 ID / Constrained opaque directory ID generated by a
     * trusted upper layer. */
...
/**
 * @brief Bot 到 Unix gateway 的只读文件恢复 DTO / Read-only file-replay DTO from the Bot to the
 * Unix gateway.
 *
...
/** @brief Unix gateway 返回给 Bot 的文件写入收据 DTO / File-ingress receipt DTO returned by the
 * Unix gateway. */
 ...
 * client.
 *
 * 它没有 namespace、mount、cgroup 或 host 权限；每次 execute 建立一条短连接并验证对端是 root-owned
 * broker。 It has no namespace, mount, cgroup, or host privileges; each execute creates a short
 * connection and verifies the peer is the root-owned broker.
 */
...
public:
     ...
     * @return allowlisted 运行态或 transport 错误 / Allowlisted operating status or a transport
     * error.
     ...
     *       journal/payload.
     */
    [[nodiscard]] Result<ClientRuntimeStatus>
    status(const ClientRuntimeStatusRequest& request) const;
     ...
     */
    [[nodiscard]] Result<ClientAddFileResult> add_file(const ClientAddFileRequest& request,
                                                       PayloadChunkSource& source) const;

    /**
     * @brief 只读恢复一个已完成的文件 ingress 收据 / Read-only replay of a completed file-ingress
     * receipt.
     * @param request 已持久的文件 import metadata / Persisted file-import metadata.
     * @return ``replayed=true`` 的 canonical 收据，或 ``not_found`` / in-doubt / conflict / A
     * canonical receipt with ``replayed=true``, or ``not_found`` / in-doubt / conflict.
