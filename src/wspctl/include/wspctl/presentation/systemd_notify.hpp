...
/**
 * @brief 向 systemd 宣告当前主进程已经可处理请求 / Notify systemd that the main process is ready to
 * serve requests.
 * @return 通知已可靠排入 systemd socket，或可诊断错误 / Notification queued to the systemd socket,
 ...
 *       clears `NOTIFY_SOCKET` so later runtime children cannot inherit the service-manager
 * capability.
 */
