...
async def get_user_active_prediction(user_id):
    """获取用户当前活跃的预测"""
    try:
        result = await crypto_repository.fetch_active_btc_prediction(
            user_id,
            datetime.now(),
        )
        ...
        async with db_connection.transaction() as connection:
            ...
            if existing_prediction:
                ...
                if end_time < datetime.now():
                    ...
                    await crypto_repository.complete_btc_predictions(
                        user_id,
                        connection=connection,
                    )
            ...
            if not account or account.total_coins < amount:
                return False, "金币不足"

            await crypto_repository.replace_btc_prediction(
                user_id,
                predict_type,
                amount,
                start_price,
...
async def check_prediction_result(user_id):
    ...
    try:
        async with db_connection.transaction() as connection:
            ...
            await crypto_repository.complete_active_btc_prediction(
                user_id,
                connection=connection,
            )
...
async def get_username_by_user_id(user_id, context):
    ...
    try:
        username = await user_repository.fetch_display_name(user_id)
        if username:
            return username
    except Exception as e:
