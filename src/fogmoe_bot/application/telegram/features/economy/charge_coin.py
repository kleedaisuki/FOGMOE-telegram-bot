...
async def _get_recharge_block_until(user_id: int) -> datetime | None:
    return await user_repository.fetch_recharge_blocked_until(user_id)
...
async def verify_and_use_code(user_id: int, code: str) -> tuple:
    ...
    try:
        async with mysql_connection.transaction() as connection:
            ...
            await economy_repository.mark_redemption_code_used(
                code_id,
                user_id,
                current_time,
                connection=connection,
...
async def topup_admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    price_label = _format_price(price_cents)
    user_name = await user_repository.fetch_display_name(target_user_id)
    ...
    if action == "block":
        ...
        await user_repository.set_recharge_blocked_until(target_user_id, blocked_until)
        await query.edit_message_text(
        ...
        async with mysql_connection.transaction() as connection:
            for _ in range(count):
                ...
                while retry_count < max_retries:
                    ...
                    exists = await economy_repository.redemption_code_exists(
                        unique_code,
                        connection=connection,
                    )
                    if not exists:
                        await economy_repository.insert_redemption_code(
                            unique_code,
                            amount,
                            connection=connection,
                        )
