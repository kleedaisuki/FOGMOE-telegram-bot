...
async def add_keyword(update: Update, chat_id: int, user_id: int, keyword: str, response: str):
    ...
    try:
        existing_keyword = await moderation_repository.group_keyword_exists(chat_id, keyword)
        ...
        if not existing_keyword:
            ...
            keyword_count = await moderation_repository.count_group_keywords(chat_id)
            
            if keyword_count >= 10:
                await update.message.reply_text("每个群组最多只能设置10个关键词，请先删除一些关键词再添加。")
                return
        
        await moderation_repository.upsert_group_keyword(
            chat_id,
            keyword,
            response,
            user_id,
...
async def del_keyword(update: Update, chat_id: int, keyword: str):
    """删除关键词"""
    try:
        rowcount = await moderation_repository.delete_group_keyword(chat_id, keyword)
