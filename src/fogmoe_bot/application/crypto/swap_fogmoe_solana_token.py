...
async def has_pending_swap_request(user_id):
    """检查用户是否有未完成的兑换请求"""
    return await crypto_repository.count_pending_swap_requests(user_id) > 0

async def get_pending_swap_request(user_id):
    """获取用户未完成的兑换请求详情"""
    result = await crypto_repository.fetch_latest_pending_swap_request(user_id)
    if result:
        return {
            "amount": result[0],
    ...
    async with lock:
        ...
        try:
            async with db_connection.transaction() as connection:
                ...
                await crypto_repository.insert_swap_request(
                    user_id,
                    username,
                    wallet_address,
                    amount,
