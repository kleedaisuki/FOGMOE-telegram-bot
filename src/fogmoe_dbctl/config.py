...
def default_config_path() -> Path:
    ...
    @return 启动目录中的 config.json / ``config.json`` in the current working directory.

    @note 默认配置属于部署目录（deployment directory），而非 Python 包安装目录。
        因此默认值相对启动目录解析；从其他目录运行时，操作者应通过 ``--config``
        指定部署配置。/
        The default configuration belongs to the deployment directory, not the Python
        package installation directory. It is therefore resolved relative to the
        startup directory; operators starting elsewhere should pass ``--config``.
    """
