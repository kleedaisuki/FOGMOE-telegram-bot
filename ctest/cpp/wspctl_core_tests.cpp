...
/**
 * @brief 创建 quota service 应已提供的 per-runtime journal layout / Create the per-runtime journal
 * layout that the quota service normally provides.
 ...
 *       ``state_root/journal``; this helper precisely simulates a ready runtime control-project
 * layout for the unprivileged unit test.
 */
void prepare_runtime_journal(const std::filesystem::path& state_root,
                             const std::string_view runtime_key) {
    std::error_code error;
    std::filesystem::create_directories(
        state_root / "runtimes" / sha256_hex(runtime_key) / "control" / "journal", error);
...
/**
 * @brief 验证有界 cgroup 元数据读取不会依赖 eofbit / Verify bounded cgroup metadata reads do not
 * depend on eofbit.
...
void test_cgroup_metadata_read() {
    ...
    /** @brief 仅需 cgroup_root 的最小 sandbox 配置 / Minimal sandbox configuration requiring only
     * cgroup_root. */
    wspctl::SandboxConfig config;
    config.cgroup_root = directory;
    const auto drained = wspctl::wait_runtime_cgroup_empty(config, std::string{kRuntimeKey});
    expect(drained.has_value(), "read a complete cgroup.events file without requiring eofbit");
    std::filesystem::remove_all(directory);
}

/**
 * @brief 验证首次 runtime 激活把缺失 cgroup 当作创建条件 / Verify first runtime activation treats a
 * missing cgroup as a creation condition.
 * @note 普通文件系统只模拟创建状态机；真实 cgroup 控制文件语义由 privileged E2E 覆盖。/
 *       A regular filesystem only simulates the creation state machine; privileged E2E covers real
 * cgroup control-file semantics.
 */
void test_missing_runtime_cgroup_is_created() {
    ...
    const std::filesystem::path wspctl_cgroup = std::filesystem::path{directory} / "wspctl";
    /** @brief 模拟的首次 runtime cgroup 路径 / Simulated first-runtime cgroup path. */
    const std::filesystem::path runtime_cgroup = wspctl_cgroup / sha256_hex(kRuntimeKey);
    ...
    {
        /** @brief 模拟已启用 controller 的父控制文件 / Parent control file simulating enabled
         * controllers. */
    ...
    const auto prepared = wspctl::prepare_runtime_cgroup(config, std::string{kRuntimeKey});
    expect(!prepared.has_value(),
           "regular filesystem cannot emulate kernel-created cgroup control files");
    expect(std::filesystem::is_directory(runtime_cgroup),
           "missing runtime cgroup advances through the normal creation path");
    if (!prepared) {
        expect(!prepared.error().message.starts_with("inspect existing runtime cgroup"),
...
void test_oci_image_identity() {
    ...
    expect(!wspctl::OciImageDigest::parse(digest_text).has_value(),
           "reject path-like image generations instead of accepting them as OCI digests");
    const auto digest = wspctl::OciImageDigest::parse("sha256:" + std::string(64U, 'a'));
    ...
    expect(root.has_value() && *root == std::filesystem::path{"/srv/fogmoe/images/sha256"} /
...
/** @brief 构造与文件写入元数据严格相同的只读恢复查询 / Construct a read-only replay query with
 * exactly the same file-ingress metadata. */
...
/**
 * @brief 写入一个既有 v1 completed execution journal / Write one existing v1 completed execution
 * journal.
 * @param state_root 测试 state root / Test state root.
 ...
 *       This is not a production writer; it fixes the old layout solely to test v2 reader's
 * lossless migration promise.
 */
[[nodiscard]] bool write_legacy_completed_execution_journal(const std::filesystem::path& state_root,
    ...
    const std::filesystem::path record_path = state_root / "runtimes" /
                                              sha256_hex(command.runtime_key) / "control" /
                                              "journal" / sha256_hex(record_material);
...
void test_protocol() {
    ...
    if (parsed_frame) {
        ...
        expect(parsed_request.has_value() && parsed_request->argv == original.argv,
               "decode request argv");
    }
    ...
    if (encoded_status_request) {
        const auto decoded_status_request =
            wspctl::decode_runtime_status_request(*encoded_status_request);
    ...
    if (encoded_status_result) {
        const auto decoded_status_result =
            wspctl::decode_runtime_status_result(*encoded_status_result);
        expect(decoded_status_result.has_value() &&
                   decoded_status_result->state == wspctl::domain::RuntimeState::ready &&
                   decoded_status_result->active_activation_id ==
                       status_result.active_activation_id &&
        ...
        const auto status_frame = wspctl::encode_frame(wspctl::MessageKind::runtime_status_result,
                                                       *encoded_status_result);
        const auto decoded_status_frame =
            status_frame ? wspctl::decode_frame(*status_frame)
    ...
    if (encoded_replay) {
        ...
        expect(decoded_replay.has_value() && decoded_replay->runtime_key == replay.runtime_key &&
                   decoded_replay->request_id == replay.request_id &&
                   decoded_replay->opaque_id == replay.opaque_id &&
                   decoded_replay->byte_size == replay.byte_size &&
                   decoded_replay->sha256 == replay.sha256,
               "round-trip activation-free file replay request");
        const auto replay_frame =
            wspctl::encode_frame(wspctl::MessageKind::payload_replay, *encoded_replay);
        const auto decoded_replay_frame =
            replay_frame ? wspctl::decode_frame(*replay_frame)
                         : wspctl::Result<wspctl::Frame>{std::unexpected(replay_frame.error())};
        expect(decoded_replay_frame.has_value() &&
                   decoded_replay_frame->kind == wspctl::MessageKind::payload_replay,
               "recognize read-only file replay frame kind");
    }
    expect(wspctl::canonical_payload_hash(file) == wspctl::canonical_payload_hash(replay),
    ...
    if (encoded_acknowledgement) {
        ...
        expect(decoded_acknowledgement.has_value() &&
                   decoded_acknowledgement->stage == acknowledgement.stage &&
    ...
    expect(!wspctl::validate_payload_begin_request(unsafe_file).has_value(),
           "reject traversal-shaped opaque file identifier");
    wspctl::PayloadReplayRequest unsafe_replay = replay;
    unsafe_replay.opaque_id = "../escape";
    expect(!wspctl::validate_payload_replay_request(unsafe_replay).has_value(),
           "reject traversal-shaped opaque replay identifier");
    std::array<int, 2> pair{-1, -1};
    const int passed_fd = open("/dev/null", O_RDONLY | O_CLOEXEC);
    expect(socketpair(AF_UNIX, SOCK_SEQPACKET | SOCK_CLOEXEC, 0, pair.data()) == 0 &&
               passed_fd >= 0,
    ...
    if (pair[0] >= 0 && pair[1] >= 0 && passed_fd >= 0) {
        ...
        expect(!wspctl::receive_frame(pair[1]).has_value(),
               "reject and close SCM_RIGHTS on public control frame");
    }
    if (passed_fd >= 0) {
        close(passed_fd);
    }
...
/**
 * @brief 绕过受测 transport 构造一个原始 SCM_RIGHTS 数据报 / Construct a raw SCM_RIGHTS datagram
 * bypassing the transport under test.
 ...
 */
[[nodiscard]] bool send_raw_scm_rights_packet(const int socket_fd,
...
/**
 * @brief 测试 fork-server SCM_RIGHTS 的五 FD 精确边界 / Test the fork-server SCM_RIGHTS exact
 * five-FD boundary.
 *
...
void test_launcher_scm_rights_contract() {
    ...
    /** @brief 用于 transport round-trip 的最小非空 wire / Minimal non-empty wire for the transport
     * round-trip. */
    constexpr std::array<std::byte, 1U> kWire{static_cast<std::byte>(0x42U)};
    /** @brief 生产上限加一，以覆盖拒绝边界 / Production limit plus one to cover the rejection
     * boundary. */
    ...
    /** @brief 正常 production send/receive 使用的 socketpair / Socketpair used by the normal
     * production send/receive path. */
    std::array<int, 2U> accepted_pair{-1, -1};
    const bool accepted_pair_ready =
        socketpair(AF_UNIX, SOCK_SEQPACKET | SOCK_CLOEXEC, 0, accepted_pair.data()) == 0;
    expect(accepted_pair_ready, "create five-FD launcher SCM_RIGHTS socketpair");
    if (descriptors_ready && accepted_pair_ready) {
        const std::span<const int> five_descriptors{descriptors.data(),
                                                    transport::kMaxFileDescriptors};
        const auto sent =
            transport::send_launcher_packet(accepted_pair[0], kWire, five_descriptors);
        ...
        if (sent) {
            ...
            if (received) {
                expect(received->fd_count == transport::kMaxFileDescriptors &&
                           received->bytes.size() == kWire.size() &&
                           received->bytes.front() == kWire.front(),
                ...
                for (std::size_t index = 0U; index < received->fd_count; ++index) {
                    ...
                    received_close_on_exec =
                        received_close_on_exec && flags >= 0 && (flags & FD_CLOEXEC) != 0;
                }
        ...
        const auto rejected_send =
            transport::send_launcher_packet(accepted_pair[0], kWire, six_descriptors);
    ...
    /** @brief hostile peer 溢出接收边界时使用的独立 socketpair / Separate socketpair for
     * hostile-peer receive-boundary overflow. */
    std::array<int, 2U> rejected_pair{-1, -1};
    const bool rejected_pair_ready =
        socketpair(AF_UNIX, SOCK_SEQPACKET | SOCK_CLOEXEC, 0, rejected_pair.data()) == 0;
    ...
    if (descriptors_ready && rejected_pair_ready) {
        ...
        if (raw_sent) {
            ...
            expect(!rejected_receive.has_value(),
                   "fail closed when hostile peer sends a sixth launcher SCM_RIGHTS FD");
            if (rejected_receive) {
...
/**
 * @brief 测试 helper 的 pidfd 终止与所有权清理 / Test helper pidfd termination and owned-FD
 * cleanup.
 *
...
void test_pidfd_terminal_signal_consumes_owned_fd() {
    ...
    if (live_child > 0) {
        /** @brief 由 production helper 消费的 live child pidfd / Live-child pidfd consumed by the
         * production helper. */
        ...
        /** @brief 用于验证实际 close 的原始 descriptor number / Original descriptor number used to
         * verify the actual close. */
        const int original_pidfd = owned_pidfd;
        /** @brief 通过 pidfd SIGKILL 得到的 terminal 判定 / Terminal result from pidfd SIGKILL. */
        const bool terminal =
            owned_pidfd >= 0 && wspctl::detail::signal_and_close_pidfd(owned_pidfd, SIGKILL);
        ...
        expect(live_waited == live_child && WIFSIGNALED(live_status) &&
                   WTERMSIG(live_status) == SIGKILL,
    ...
    /** @brief 已退出 child，用于 ESRCH 可接受终态 / Exited child for the accepted ESRCH terminal
     * state. */
    const pid_t exited_child = fork();
    expect(exited_child >= 0, "fork exited child for pidfd ESRCH test");
    if (exited_child > 0) {
        /** @brief child 尚可 wait 时取得的 pidfd / pidfd acquired while the child is still
         * waitable. */
        ...
        expect(exited_waited == exited_child && WIFEXITED(exited_status),
               "reap exited pidfd test child");
        /** @brief 用于验证 ESRCH path close 的 descriptor number / Descriptor number used to verify
         * close on the ESRCH path. */
        ...
        const bool already_terminal =
            exited_pidfd >= 0 && wspctl::detail::signal_and_close_pidfd(exited_pidfd, SIGKILL);
    ...
    /** @brief 被外部关闭的 pidfd，覆盖 syscall 失败仍消费所有权 / Externally closed pidfd covering
     * ownership consumption after syscall failure. */
    const pid_t invalid_child = fork();
    expect(invalid_child >= 0, "fork child for invalid-pidfd error path");
    if (invalid_child > 0) {
        /** @brief 故意在 helper 前关闭的 child pidfd / Child pidfd deliberately closed before the
         * helper. */
        ...
        if (invalid_pidfd >= 0) {
            /** @brief 用于验证 error path 无泄漏的原 descriptor number / Original descriptor number
             * used to verify no error-path leak. */
            const int original_pidfd = invalid_pidfd;
            static_cast<void>(close(invalid_pidfd));
            expect(!wspctl::detail::signal_and_close_pidfd(invalid_pidfd, SIGKILL) &&
                       invalid_pidfd == -1,
        ...
        int invalid_status{};
        /** @brief invalid-pidfd child 的 waitpid 返回值 / waitpid return value for the
         * invalid-pidfd child. */
        const pid_t invalid_waited = waitpid(invalid_child, &invalid_status, 0);
        expect(invalid_waited == invalid_child && WIFSIGNALED(invalid_status) &&
                   WTERMSIG(invalid_status) == SIGKILL,
...
/** @brief 测试 journal pending/completed/conflict 语义 / Test journal pending/completed/conflict
 * semantics. */
void test_journal() {
    ...
    expect(pending.has_value() && pending->has_value() &&
               (*pending)->state == wspctl::JournalState::pending,
    ...
    expect(completed.has_value() && completed->has_value() &&
               (*completed)->operation == wspctl::JournalOperation::execution &&
    ...
    expect(journal.begin(mismatched_execution).has_value(),
           "create pending journal for mismatched execution receipt");
    ...
    expect(journal.begin(replayed_execution).has_value(),
           "create pending journal for replayed execution receipt");
    ...
    expect((*completed)->request_hash != conflicting.request_hash,
           "detect differing semantic hash");
    ...
    const auto migrated_legacy =
        journal.lookup(legacy_command.runtime_key, legacy_command.request_id);
    ...
    expect(write_legacy_completed_execution_journal(directory, malformed_legacy_command,
                                                    malformed_legacy_result),
           "write a v1 fixture with mismatched receipt identity");
    expect(
        !journal.lookup(malformed_legacy_command.runtime_key, malformed_legacy_command.request_id)
             .has_value(),
    ...
    expect(journal.complete_payload(file, file_result).has_value(),
           "persist completed file journal");
    ...
    expect(journal.begin_payload(replayed_file).has_value(),
           "create pending journal for replayed file receipt");
    ...
    expect(journal.begin_payload(race_file).has_value(),
           "second contender persists a pending file journal");
    ...
    expect(journal.complete_payload(race_file, race_receipt).has_value(),
           "complete raced file journal");
    ...
    expect(!journal.begin(cross_operation).has_value(),
           "reject journal operation reuse under one request ID");
    std::filesystem::remove_all(directory);
}

/**
 * @brief 测试只读 attachment replay 的无状态缺失与对象证明语义 / Test attachment replay's stateless
 * miss and object-proof semantics.
 *
...
void test_payload_replay_recovery_contract() {
    /** @brief isolated durable-state test root template / Isolated durable-state test-root
     * template. */
    ...
    /** @brief replay request with the same durable identity as file ingress / Replay request
     * sharing the durable identity of file ingress. */
    const wspctl::PayloadReplayRequest replay = file_replay_request();
    ...
    expect(
        !std::filesystem::exists(std::filesystem::path(directory) / "runtimes", no_write_error) &&
            !no_write_error,
        "missing replay receipt does not create journal/runtime state");

    prepare_runtime_journal(directory, replay.runtime_key);
    /** @brief journal backed by the explicitly provisioned test layout / Journal backed by the
     * explicitly provisioned test layout. */
    const wspctl::Journal journal(directory);
    const wspctl::PayloadBeginRequest begin = file_request();
    expect(journal.begin_payload(begin).has_value(),
           "persist completed-replay payload pending record");
    ...
    expect(journal.complete_payload(begin, stored_receipt).has_value(),
           "complete replay payload journal record");
    ...
    /** @brief test-only runtime binding whose workspace needs no XFS for no-follow digest
     * verification / Test-only runtime binding whose workspace needs no XFS for no-follow digest
     * verification. */
    ...
    const std::filesystem::path payload_parent =
        binding.workspace_dir / "upper" / "uploads" / replay.opaque_id;
    ...
    {
        /** @brief deliberately digest-mismatched payload output stream / Deliberately
         * digest-mismatched payload output stream. */
        std::ofstream output(payload_path, std::ios::binary | std::ios::trunc);
        output << "HELLO WORLD";
        expect(output.good(), "write same-size digest-mismatched replay payload");
    }
    const auto mismatched_object =
        wspctl::detail::verify_replayable_payload_object(binding, replay, *resolved);
    expect(!mismatched_object &&
               mismatched_object.error().code == wspctl::ErrorCode::invocation_in_doubt,
           "persistent replay object SHA-256 mismatch is invocation-in-doubt");
    std::filesystem::remove_all(directory);
}

/** @brief 测试 runtime gate 拒绝共享 workspace 并发 / Test runtime gate rejects concurrent
...
void test_runtime_gate() {
    ...
    expect(!second.has_value() && second.error().code == wspctl::ErrorCode::busy,
           "reject concurrent runtime lease");
}

/** @brief 测试 quota 配置拒绝 generic/root filesystem fallback / Test quota configuration rejects
 * generic/root-filesystem fallback. */
void test_xfs_quota_fail_closed_contract() {
    ...
    expect(!malformed_result &&
               malformed_result.error().code == wspctl::ErrorCode::invalid_argument,
...
/** @brief 测试 timeout、输出截断和信号退出码 / Test timeout, output truncation, and signal exit
 * code. */
void test_supervisor() {
    ...
    expect(environment_result.has_value() &&
    ...
    expect(std::filesystem::create_directories(nested_cwd), "create nested supervisor cwd");
    ...
    expect(nested_cwd_result.has_value() && nested_cwd_result->exit_code == 0 &&
               nested_cwd_result->stdout_data == nested_cwd.string() + "\n",
           "resolve nested cwd beneath the pinned workspace FD");
    /** @brief 指向 workspace 内目录的 cwd symlink / Cwd symlink pointing to a directory inside the
     * workspace. */
    const std::filesystem::path cwd_symlink = std::filesystem::path(workspace) / "cwd-link";
    expect(symlink(nested_cwd.c_str(), cwd_symlink.c_str()) == 0, "create supervisor cwd symlink");
    ...
    expect(symlink_cwd_result.has_value() && symlink_cwd_result->exit_code == 126 &&
    ...
    expect(missing_cwd_result.has_value() && missing_cwd_result->exit_code == 126 &&
    ...
    expect(timeout_result.has_value() && timeout_result->timed_out &&
               !timeout_result->exit_code.has_value(),
           "enforce timeout with null exit code");
    ...
    expect(output_result.has_value() && output_result->truncated &&
               output_result->stdout_data.size() + output_result->stderr_data.size() <=
                   output_request.output_limit,
    ...
    expect(signal_result.has_value() && signal_result->exit_code.has_value() &&
               *signal_result->exit_code == 137,
    ...
    if (malformed_output) {
        ...
        if (encoded) {
            ...
            expect(decoded.has_value() && decoded->stdout_data == "??",
                   "normalized output round-trips through protocol");
        }
        ...
        if (journal_directory != nullptr) {
            ...
            expect(journal.begin(malformed_output_request).has_value(),
                   "persist sanitized-output pending journal");
            expect(journal.complete(malformed_output_request, *malformed_output).has_value(),
                   "persist sanitized-output completed journal");
            const auto replay = journal.lookup(malformed_output_request.runtime_key,
                                               malformed_output_request.request_id);
            expect(replay.has_value() && replay->has_value() &&
                       (*replay)->execution_result.has_value() &&
    ...
    expect(unicode_boundary.has_value() && unicode_boundary->truncated &&
               unicode_boundary->stdout_data.empty(),
    ...
    expect(malformed_sequence.has_value() &&
               malformed_sequence->stdout_data == malformed_sequence_expected,
    ...
    expect(file_published.has_value() &&
               file_published->path == "/workspace/uploads/" + file.opaque_id + "/payload",
    ...
    const std::string published_content{std::istreambuf_iterator<char>(published_file),
    ...
    expect(stat(published_path.c_str(), &published_metadata) == 0 &&
               S_ISREG(published_metadata.st_mode) && (published_metadata.st_mode & 0777) == 0600,
    ...
    expect(chmod(uploads_path.c_str(), 0777) == 0,
           "make a pre-existing uploads directory intentionally permissive");
    ...
    expect(stat(uploads_path.c_str(), &uploads_metadata) == 0 &&
               (uploads_metadata.st_mode & 0777) == 0700,
    ...
    expect(!std::filesystem::exists(std::filesystem::path(workspace) / "uploads" /
                                    aborted_file.opaque_id / "payload"),
