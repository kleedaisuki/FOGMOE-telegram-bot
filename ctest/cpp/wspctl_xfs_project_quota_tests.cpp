 ...
 * @brief XFS project quota 的显式特权集成测试 / Explicit privileged integration test for XFS project quota.
 *
...
/** @brief disposable state-root parent 的环境变量 / Environment variable for the disposable state-root parent. */
...
[[nodiscard]] int unavailable(const std::string_view reason) {
    if (quota_test_is_required()) {
        std::cerr << "FAIL: " << kRequireEnvironment << "=1 but XFS quota test cannot run: " << reason << '\n';
        return EXIT_FAILURE;
    }
 ...
 */
[[nodiscard]] wspctl::Result<std::filesystem::path> canonical_private_parent(
    const std::filesystem::path& configured_parent) {
    if (!configured_parent.is_absolute()) {
        return std::unexpected(wspctl::make_error(wspctl::ErrorCode::invalid_argument, "XFS quota test parent must be absolute"));
    }
    ...
    if (error || parent == parent.root_path()) {
        return std::unexpected(wspctl::make_error(wspctl::ErrorCode::invalid_argument, "XFS quota test parent is not a usable non-root directory"));
    }
    /** @brief parent metadata / 父目录元数据。 */
    struct stat metadata {};
    if (lstat(parent.c_str(), &metadata) != 0 || !S_ISDIR(metadata.st_mode) || metadata.st_uid != 0U ||
        metadata.st_gid != 0U || (metadata.st_mode & (S_IWGRP | S_IWOTH)) != 0) {
        return std::unexpected(wspctl::make_error(
            wspctl::ErrorCode::permission_denied,
 ...
 */
[[nodiscard]] wspctl::Result<std::filesystem::path> create_test_state_root(const std::filesystem::path& parent) {
    ...
    if (created == nullptr) {
        return std::unexpected(wspctl::errno_error(wspctl::ErrorCode::io_failure, "mkdtemp XFS quota state root"));
    }
    const std::filesystem::path state_root(created);
    if (chmod(state_root.c_str(), 0700) != 0) {
        return std::unexpected(wspctl::errno_error(wspctl::ErrorCode::io_failure, "chmod XFS quota test state root"));
    }
    return state_root;
}
 ...
 */
[[nodiscard]] wspctl::Result<void> remove_test_state_root(
    const std::filesystem::path& parent,
    const std::filesystem::path& state_root) {
    if (state_root.parent_path() != parent || !state_root.filename().string().starts_with("wspctl-xfs-quota-")) {
        return std::unexpected(wspctl::make_error(
            wspctl::ErrorCode::invalid_argument,
 ...
 *       that capability while retaining the other credentials needed to access the private test tree.
 */
...
/**
 * @brief 在无 quota bypass capability 的 child 中验证 inode hard limit / Verify the inode hard limit in a child without quota bypass capability.
...
/**
 * @brief 在无 quota bypass capability 的 child 中验证 byte hard limit / Verify the byte hard limit in a child without quota bypass capability.
 * @param upper_dir project-inheriting upper directory / 带 project inheritance 的 upper 目录。
 * @return child 成功时为 0，否则为非零 / 0 when the child succeeds, nonzero otherwise.
 */
[[nodiscard]] int verify_byte_hard_limit_child(const std::filesystem::path& upper_dir) {
    ...
    /** @brief posix_fallocate 返回的 errno-style status / errno-style status returned by posix_fallocate. */
 ...
 */
[[nodiscard]] wspctl::Result<void> run_boundary_probe(
    const std::filesystem::path& upper_dir,
    int (*probe)(const std::filesystem::path&),
    const std::string_view name) {
    const pid_t child = fork();
    if (child < 0) {
        return std::unexpected(wspctl::errno_error(wspctl::ErrorCode::child_failure, "fork XFS quota boundary probe"));
    }
    if (child == 0) {
    ...
    if (waited != child) {
        return std::unexpected(wspctl::errno_error(wspctl::ErrorCode::child_failure, "wait XFS quota boundary probe"));
    }
    if (!WIFEXITED(status) || WEXITSTATUS(status) != EXIT_SUCCESS) {
        return std::unexpected(wspctl::make_error(
            wspctl::ErrorCode::child_failure,
...
[[nodiscard]] int run_xfs_project_quota_test() {
    ...
    const std::optional<std::filesystem::path> configured_parent = environment_path(kParentEnvironment);
    ...
    do {
        ...
        if (const auto preflight = wspctl::preflight_xfs_project_quota(config, *state_root); !preflight) {
        ...
        if (const auto inode_probe = run_boundary_probe(
                binding->workspace_dir / "upper",
                verify_inode_hard_limit_child,
                "XFS inode hard-limit probe"); !inode_probe) {
            std::cerr << "FAIL: " << inode_probe.error().message << '\n';
            break;
        }
        if (const auto byte_probe = run_boundary_probe(
                binding->workspace_dir / "upper",
                verify_byte_hard_limit_child,
                "XFS byte hard-limit probe"); !byte_probe) {
 ...
 */
int main() {
    return run_xfs_project_quota_test();
}
